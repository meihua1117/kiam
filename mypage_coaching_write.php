                ...
                <td>
                  ...
                  <select name="coty_id" id="coty_id" onchange="cotyIDChanged()" style="height:35px;width:75%; padding-left:5px;">
                    ...
                    ?>
                        ...
                        <option value="<?= $coaching_data["coty_id"] ?>_<?= $coaching_data["mem_name"] ?>_<?= $coaching_data["mem_code"] ?>_<?= $coaching_data["mem_id"] ?>" <? if ($coaching_data["coty_id"] == $coaching_info_data['coty_id']) {
                                                                                                                                                                            echo selected;
  ...
  <script>
    ...
    $(function() {
      ...
      $('#saveBtn').on("click", function() {
        ...
        if ($("#coaching_time").val().trim() == "") {
          alert("값을 정확히 입력하세요.");
          $("#coaching_time").focus();
          return;
        };
        if ($("#coaching_title").val().trim() == "") {
          alert("코칭제목을 정확히 입력하세요.");
          $("#coaching_title").focus();
          return;
        };
        if ($("#coaching_content").val().trim() == "") {
          alert("코칭내용을 정확히 입력하세요.");
          $("#coaching_content").focus();
          return;
        };
        if ($("#coach_value").val().trim() == "") {
          alert("코치평가 정확히 입력하세요.");
          $("#coach_value").focus();
          return;
        };
        if ($("#home_work").val().trim() == "") {
          alert("과제안내를 정확히 입력하세요.");
          $("#home_work").focus();
          return;
        };
        if ($("#coach_comment").val().trim() == "") {
          alert("코치의견을 정확히 입력하세요.");
          $("#coach_comment").focus();
          return;
        };


        if ($("#coach_value").val() > 5) {
          alert("코치평가 정확히 입력하세요. 1 - 5까지의 숫자여야 합니다.");
          $("#coach_value").focus();
          return;
        };
