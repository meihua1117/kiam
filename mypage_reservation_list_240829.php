	...
	<table class="table table-bordered" style="width: 97%;">
		<tbody>
			<tr class="hide_spec">
				...
				<textarea name="step_send_id_count" id="step_send_id_count" style="width:90%; height:50px;color: red;font-size: 12px" readonly="" data-num="0" placeholder="0건"></textarea></td>
...
function deleteMultiRow() {
    ...
    if(confirm('삭제하시겠습니까?')) {
        $.ajax({
            ...
            dataType:"json",
            data:{admin:0, delete_name:"mypage_reservation_list", id:no_array.toString()},
            success: function(data){
                console.log(data);
                if(data == 1){
                    alert('삭제 되었습니다.');
...
function send_step_sms(){
	...
	$.ajax({
		...
		dataType:"json",
		data:{send_ids:send_ids, sms_idx:sms_idx, type:"step"},
		success:function (data) {
			console.log(data);
			alert("전송되었습니다.");
			location.reload();
