...
<body>
    ...
    <form name="group_detail_form" action="" method="post">
        ...
        <table class="list_table" width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td style="width:25%;"><a href="javascript:void(0)" onclick="order_sort(group_detail_form,'grp_2',group_detail_form.order_status.value)">소그룹명<? if ($_REQUEST['order_name'] == "grp_2") {
                ...
                <td style="width:25%;">
                    ...
                    <li style="font-size:13px;font-family:'Nanum Gothic';"><label for="all_id">선택</label>
                        <a href="javascript:void(0)" onclick="order_sort(group_detail_form,'name',group_detail_form.order_status.value)">이름<? if ($_REQUEST['order_name'] == "name") {
                                                                                                                                                echo $_REQUEST['order_status'] == "desc" ? '▼' : '▲';
                                                                                                                                            } else {
                                                                                                                                                echo '▼';
                                                                                                                                            } ?></a>
            ...
            ?>
                    <tr>
                        <td class="g_dt_grp_<?= $i ?> stop-dragging"><a href="javascript:void(0)" onclick="group_detail_form.grp_2.value='<?= $row['grp_2'] ?>';group_detail_form.submit();"><?= $row['grp_2'] ?></a></td>
                        <td class="g_dt_grp_<?= $i ?> stop-dragging" style="display:none;"><input type="text" name="xiao_grp" value="<?= $row['grp_2'] ?>" /></td>
                        <td class="g_dt_name_<?= $i ?> stop-dragging">
                            <li><!--<input type="checkbox" name="name_box" value="<?= $row['recv_num'] ?>" id='lbox_<?= $i ?>' onclick="box_ed('<?= $i ?>')" />--></li>
                            <li><label for="lbox_<?= $i ?>"><?= $row['name'] ?></label></li>
                        </td>
                        <td class="g_dt_name_<?= $i ?> stop-dragging" style="display:none;"><input type="text" name="num_name" value="<?= $row['name'] ?>" /></td>
                        <td class="g_dt_num_<?= $i ?> text"><span><?= $row['recv_num'] ?></span></td>
                        <!--
            <td class="stop-dragging">
            	<a href="javascript:void(0)" class="stop-dragging modify_btn_<?= $i ?>" onclick="g_dt_show_cencle('g_dt_name_','g_dt_num_','modify_btn_','<?= $i ?>','g_dt_grp_')">수정</a>                
                <a href="javascript:void(0)" class="stop-dragging modify_btn_<?= $i ?>" style="display:none;" onclick="g_dt_fun('modify','<?= $_REQUEST['grp_id'] ?>','<?= $row['idx'] ?>','<?= $i ?>')">수정</a>               
                <a href="javascript:void(0)" class="stop-dragging" onclick="g_dt_fun('del','<?= $_REQUEST['grp_id'] ?>','<?= $row['idx'] ?>','<?= $i ?>')">삭제</a>
            </td>
            -->
                    </tr>
                <?
                ...
                <tr>
                    <td colspan="4">
                        ...
                        ?>
                        <a href="javascript:;;" id="copyBtn">선택</a>
                    </td>
                </tr>
            <?
            } else {
...
<script>
    $(function() {
        $('#copyBtn').on("click", function() {
            ...
            console.log(selectionText.getRangeAt(0).toString());
        });
    });
    ...
    function selectText() {
        ...
        data = $.trim(content.textContent);


        console.log(data);
        console.log($.trim(content.querySelector('span').textContent));
