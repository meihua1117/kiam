...
function deleteMultiRow() {
    ...
    if(confirm('삭제하시겠습니까?')) {
        $.ajax({
            ...
            dataType:"json",
            data:{admin:0, delete_name:"mypage_link_list", id:no_array.toString()},
...
<script>
	...
	function send_req_link(){
		...
		$.ajax({
			...
			dataType:"json",
			data:{send_ids:send_ids, req_idx:req_idx, type:"reqlink"},
			success:function (data) {
				console.log(data);
				alert("전송되었습니다.");
				location.reload();
	...
	function delete_all(){
		...
		if(msg){
			$.ajax({
				type:"POST",
				url:"/admin/ajax/delete_func.php",
				data:{admin:0, delete_name:"mypage_link_list", mem_id:'<?=$_SESSION['one_member_id']?>'},
				success:function(){
					alert('삭제되었습니다.');
					location.reload();
				},
