...
if($row['daily_cnt']){ $daily_cnt = $row['daily_cnt']; }
else{
...
$(function() {
    ...
    $('#saveBtn').on("click", function () {
        ...
        $.ajax({
            ...
            data:{grp_id:grp_id, send_num:send_num, iam:false},
...
function apply() {
    ...
    location.href = '<? if(strpos($_SERVER['REQUEST_URI'], "?daily_cnt") !== false) {$link = explode("?daily_cnt", $_SERVER['REQUEST_URI']); echo trim($link[0]); }  else{ echo $_SERVER['REQUEST_URI'];}?>' + '?daily_cnt=' + daily_cnt + '&address_cnt=' + $("#address_cnt").text() + '&address_name=' + $("#address_name").val() + '&address_idx=' + $("#address_idx").val() + '&start_send=' + start_send + '&htime=' + htime + '&mtime=' + mtime + '&send_num=' + send_num + '&week_end=' + week_end_state + '&msg_mode=' + msg_mode + '&max_count=' + max_count;
    <?}
    else{?>
    // location.href = '<?=$_SERVER['REQUEST_URI']?>' + '&daily_cnt=' + daily_cnt + '&start_send=' + start_send + '&htime=' + htime + '&mtime=' + mtime + '&send_num=' + send_num;
    location.href = '<? if(strpos($_SERVER['REQUEST_URI'], "&daily_cnt") !== false) {$link = explode("&daily_cnt", $_SERVER['REQUEST_URI']); echo trim($link[0]); }  else{ echo $_SERVER['REQUEST_URI'];}?>' + '&daily_cnt=' + daily_cnt + '&start_send=' + start_send + '&htime=' + htime + '&mtime=' + mtime + '&send_num=' + send_num + '&week_end=' + week_end_state + '&msg_mode=' + msg_mode + '&max_count=' + max_count;
    <?}?>
}
function cancel_set(){
       ...
       <div class="m_body">
        <form name="sub_4_form" id="sub_4_form" action="mypage.proc.php" method="post" enctype="multipart/form-data">
            ...
            <div class="popupbox" style="height: 64px;width: 260px;color: black;left: 195px;top: -37px;">디비를 매일 발송가능한 숫자로 나누어 매일 발송할 수 있도록 자동화 등록하는 기능입니다.<br><br>
                      <a class = "detail_view" style="color: blue;" href="https://url.kr/i35Dq2" target="_blank">[자세히 보기]</a>
                    </div>
                </div>
            </li>
                ...
                <div class="p1">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="javascript:apply();"
                                        style="text-decoration-line: blink;background-color: #5f72e6;padding: 3px;color: white;border-radius: 5px;">적용</a>
                    ...
                    <?
                    $hide_msg = "";
                    if((isset($_GET['msg_mode']) && $_GET['msg_mode'] != '0') || $row['step_sms_idx'] != 0){ $hide_msg = "hidden"; }?>
                    ...
                    <tr class="msg_info" <?=$hide_msg?>>
                        ...
                        <td>
                            <input type="file" name="upimage2" onChange="onUploadImgChange(this);sub_4_form.action='up_image_.php?i=0&k=2&target=upimage_str2';sub_4_form.target='excel_iframe';sub_4_form.submit();" /></div>
                            <?php if($row['jpg2'] != "") {?>
