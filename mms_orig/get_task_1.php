...
if(!$token || !check_token($user_id, $token)){
	echo "{\"txt\":\"\",\"reqid\":\"\",\"delay\":\"\",\"close\":\"\",\"idx\":\"\",\"type\":\"\",\"title\":\"\",\"jpg\":\"\",\"jpg1\":\"\",\"jpg2\":\"\",\"pnum\":[],\"token_res\":\"0\"}";
}
else{
	...
	//$sql = "select * from Gn_MMS where 1 and result > 0 and send_num = '".$userId."' order by idx limit 1"; //������ ���� ����, �ش� ������ ��ȣ
	...
	if(strpos($row[recv_num],",")){
		...
		echo "{\"txt\":$msg,\"reqid\":\"$row[uni_id]\",\"delay\":\"$row[delay]\",\"close\":\"24\",\"idx\":\"$row[idx]\",\"type\":\"$row[type]\",\"title\":$title,\"jpg\":\"$row[jpg]\",\"jpg1\":\"$row[jpg1]\",\"jpg2\":\"$row[jpg2]\",\"pnum\":[$string],\"token_res\":\"1\"}";
		...
		echo "{\"txt\":$msg,\"reqid\":\"$row[uni_id]\",\"type\":\"$row[type]\",\"idx\":\"$row[idx]\",\"delay\":\"$row[delay]\",\"close\":\"24\",\"title\":$title,\"jpg\":\"$row[jpg]\",\"jpg1\":\"$row[jpg1]\",\"jpg2\":\"$row[jpg2]\",\"pnum\":[{\"bnc\":\"$url\",\"num\":\"$row[recv_num]\",\"rep\":\"$row[replacement1]\",\"rep1\":\"$row[replacement2]\"}],\"token_res\":\"1\"}";
	...
	if($row[idx]) {
		...
		//$query_j = mysqli_query($self_con,$sql_j);
	}
