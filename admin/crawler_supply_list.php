...
function goPage(pgNum) {
  location.href = '?nowPage='+pgNum+'&user_id=<?=$user_id?>&data_type=<?=$data_type?>&company_type=<?=$company_type?>&cell=<?=$cell?>&address=<?=$address?>&search_key=<?=$search_key?>&search_start_date=<?=$search_start_date?>&search_end_date=<?=$search_end_date?>&orderField=<?=$orderField?>&dir=<?=$dir?>';
}
$(function() {
                ...
                <div class="input-group" >
                  ...
                  <div class="form-group">
                    <select name="data_type" class="form-control input-sm " >
                          ...
                          <option value="쇼핑" <?if($_REQUEST['data_type'] == "쇼핑") echo "selected"?>>N쇼핑</option>
                          <option value="지마켓" <?if($_REQUEST['data_type'] == "지마켓") echo "selected"?>>G쇼핑</option>
                ...
                <div class="box-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <thead>
                      <tr>
                        ...
                        <th>주소</th>
                        <th>웹주소</th>
                        <th><a href="?nowPage=<?=$nowPage?>&user_id=<?=$user_id?>&data_type=<?=$data_type?>&company_type=<?=$company_type?>&cell=<?=$cell?>&address=<?=$address?>&search_key=<?=$search_key?>&search_start_date=<?=$search_start_date?>&search_end_date=<?=$search_end_date?>&orderField=tag&dir=<?=$dir?>" class="sort-by">태그</a></th>
                        <th><a href="?nowPage=<?=$nowPage?>&user_id=<?=$user_id?>&data_type=<?=$data_type?>&company_type=<?=$company_type?>&cell=<?=$cell?>&address=<?=$address?>&search_key=<?=$search_key?>&search_start_date=<?=$search_start_date?>&search_end_date=<?=$search_end_date?>&orderField=info&dir=<?=$dir?>" class="sort-by">폰관련정보</a></th>
                        <th>수집일</th>
                      </tr>
                    </thead>
                    <tbody>
                  ...
                  if($search_key){
                    if(strpos($search_key, ' ') !== false){
                      ...
                      $search_key1 = $key_arr[0];
                      $search_key2 = $key_arr[1];
                      $searchStr .= $search_key ? " AND (a.keyword LIKE '%".$search_key1."%' and a.keyword LIKE '%".$search_key2."%')" : null;
                    }
                    else{
                  ...
                  if($address) {
                    if(strpos($address, ' ') !== false){
                      ...
                      $search_address1 = $address_arr[0];
                      $search_address2 = $address_arr[1];
                      $searchStr .= $address ? " AND (a.address LIKE '%".$search_address1."%' and a.address LIKE '%".$search_address2."%')" : null;
                    }
                    else{
                  ...
                  ?>
                      <tr>
                        ...
                        <td>
                          <?if($row['info'] != ""){?>
                          <a href="javascript:openInfo('<?=$row['info']?>', '<?=$row['seq']?>');"><? echo (mb_strlen($row['info'], "UTF-8") > 8)?mb_substr($row['info'], 0, 8, "UTF-8"):$row['info'];?></a>
                          <?}
                          else{?>
                          <a href="javascript:openInfo('', '<?=$row['seq']?>');">폰관련정보</a>
                          <?}?>
                        </td>
                            <!--기부폰-->
                        <td><?=$row['regdate']?></td>
          ...
          <div class="modal-dialog modal-sm" role="document" style="width:100%;max-width:768px;">
              <div class="modal-content">
                  ...
                  <div class="modal-body">
                      ...
                      <input type="hidden" name="db_idx" id="db_idx" value="0">
                  </div>
                  <div class="modal-footer" style="text-align:center;" hidden>
                    <a class="btn btn-default btn-submit" style="border-radius: 5px;width:30%;font-size:15px;background-color: #ff0066;color:white;" href="javascript:save_info()">저장</a>
                </div>
              </div>
          </div>
      </div>
...
function openInfo(info, id=''){
  $("#txtInfo").val(info);
  if(id != ""){
    $("#txtInfo").prop('readonly', false);
    $(".modal-footer").show();
    $("#db_idx").val(id);
  }
  else{
    $("#txtInfo").prop('readonly', true);
    $(".modal-footer").hide();
  }
...
function save_info(){
  ...
  $.ajax({
		...
		dataType:"json",
		data:{mode:"change_info",mem_code:id,info:info},
		success:function(data){
			alert('변경이 완료되었습니다.');
      location.reload();
		},
