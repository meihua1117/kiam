    ...
    if($type == "cha"){
        ...
        if(mysqli_num_rows($res_chk) == 0){
            ...
            if($data['mem_point'] * 1 < $point){
                ...
                $sql = "insert into Gn_Item_Pay_Result set buyer_id='{$mem_id}', buyer_tel='{$data['mem_phone']}', item_name='씨드포인트차감', item_price={$point}, pay_percent=90, current_point={$current_point}, current_cash={$data['mem_cash']}, pay_status='Y', VACT_InputName='{$data['mem_name']}', type='minus', seller_id='{$_SESSION['one_member_id']}', pay_method='관리자씨드차감', pay_date=now(), point_val=1,billdate=now()";
                // echo $sql; exit;
        ...
        // if(mysqli_num_rows($res_chk) == 0){
        $current_point = $data['mem_point'] * 1 + $point;
        $sql = "insert into Gn_Item_Pay_Result set buyer_id='{$mem_id}', buyer_tel='{$data['mem_phone']}', item_name='씨드포인트충전', item_price={$point}, pay_percent=90, current_point={$current_point}, current_cash={$data['mem_cash']}, pay_status='Y', VACT_InputName='{$data['mem_name']}', type='buy', seller_id='{$_SESSION['one_member_id']}', pay_method='관리자씨드충전', pay_date=now(), point_val=1,billdate=now()";
        // echo $sql; exit;
        mysqli_query($self_con,$sql);
        $sql_update = "update Gn_Member set mem_point={$current_point} where mem_id='{$mem_id}'";
        mysqli_query($self_con,$sql_update);
...
if(isset($_POST['mem_id']) && isset($_POST['share_cash']) && $_POST['share_cash']){
    ...
    if($type == "chacash"){
        ...
        $current_cash = $data['mem_cash'] * 1 + $cash;
        $sql = "insert into Gn_Item_Pay_Result set buyer_id='{$mem_id}', buyer_tel='{$data['mem_phone']}', item_name='캐시포인트충전', item_price={$cash}, pay_percent=90, current_point={$data['mem_point']}, current_cash={$current_cash}, pay_status='Y', VACT_InputName='{$data['mem_name']}', type='buy', seller_id='{$_SESSION['one_member_id']}', pay_method='관리자캐시충전', pay_date=now(), point_val=1,billdate=now()";
        // echo $sql; exit;
        mysqli_query($self_con,$sql);
        $sql_update = "update Gn_Member set mem_cash={$current_cash} where mem_id='{$mem_id}'";
        mysqli_query($self_con,$sql_update);
