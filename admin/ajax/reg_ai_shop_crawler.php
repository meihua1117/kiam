...
if($delete){
    ...
    if (strpos($reg_search_addr, ',') !== false) {
        ...
        echo json_encode(array("status"=>$res,"set_id"=>$set_idxs[0], "set_ids"=>implode(',',$set_idxs), "reg_search_addr"=>implode(',',$search_addrs)));
    } else {
        ...
        $res = mysqli_query($self_con,$sql_reg);
        $set_idx = mysqli_insert_id($self_con);
        echo json_encode(array("status"=>$res,"set_id"=>$set_idx, "set_ids"=>$set_idx, "reg_search_addr"=>$reg_search_addr));
    }
}
?>