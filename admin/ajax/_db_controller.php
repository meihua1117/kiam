...
if($mode == "delete_name_card") {
    ...
    $res_chk_use = mysql_query($sql_chk_use);
    $row_chk_use = mysql_fetch_array($res_chk_use);
    if($row_chk_use[0]){
        echo 1;
        exit;
    }
    ...
    $res_url = mysql_query($sql_short_url);
    ...
    foreach($result111 as $val){
        ...
        $res_state = mysql_query($sql_del_state) or die(mysql_error());
        ...
        $res_con = mysql_query($sql_del_con) or die(mysql_error());
    }
...
if($mode == "update_name_card - call_click") {
    if($call_click == "Y"){
        ...
        $res_shop_id = mysql_query($sql_shop_id);
        $row_shop_id = mysql_fetch_array($res_shop_id);
        ...
        $res_gid = mysql_query($sql_card_gid);
...
if($mode == "update-contents-sample-display") {
    ...
    $res = mysql_query($sql);
    $cont_row = mysql_fetch_assoc($res);
