...
$fp = fopen("test.txt","w+");
fwrite($fp, $_POST[one_id]);
fwrite($fp, '\/n');
if($_POST[one_id] && $_POST[mem_pass])
{
	$mem_pass=$_POST[one_pwd];
	$sql="select mem_code,mem_leb from Gn_Member where mem_id = '$_POST[one_id]' and web_pwd='$_POST[mem_pass]' and mem_code='$_POST[mem_code]' ";
	fwrite($fp, $sql);
	fwrite($fp, '\/n');
	...
	if($row[mem_code])
	{
	    ...
	    if($row[0] != "") {
			...
			if($sub_domain) {
				...
				if ($row[0] != "") {
					...
					$_SESSION[one_member_subadmin_domain] = $_SERVER[HTTP_HOST];
				}
			}
		}
		//user_session_check($_POST[one_id]);
		...
		if($sub_domain){?>
			<script language='javascript'>
				location.href="<?=$sub_domain?>";
			</script>
		<?}
		else{?>
			<script language="javascript">
				window.parent.location.replace('/sub_5.php');
			</script>
    	<?}
	}
	else
