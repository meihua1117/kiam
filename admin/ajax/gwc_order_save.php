...
if($type == "delivery_save"){
    ...
    for($i = 0; $i < count($no_arr); $i++){
        ...
        if($row_id)
        {
            ...
            $sql_pay = "select sum(TotPrice) as all_money from tjd_pay_result_gwc where cash_prod_pay=0 and buyer_id='{$row_id['buyer_id']}' and date>'$date_pre_month' and end_status='Y'";
