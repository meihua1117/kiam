    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            수집고객 정보관리
            <small>수집고객의 정보를 관리합니다.</small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">수집고객 정보관리</li>
          </ol>
        </section>
        ...
        <section class="content">
          <div class="row" id="toolbox">
            <div class="col-xs-12" style="padding-bottom:20px">
              <button class="btn btn-primary pull-right" style="margin-right: 5px;border: none;" onclick="location='customer_get_list.php';return false;">수집고객 정보관리</button>
              <button class="btn btn-primary pull-right" style="margin-right: 5px;background-color:#aba9a9;border: none;" onclick="location='customer_req_list.php';return false;">신청고객 정보관리</button>
          ...
          <div class="row">
              <div class="box">
                <div class="box-body">
                  ...
                  // 검색 조건을 적용한다.
                  $searchStr .= $search_userid ? " AND user_id LIKE '%".$search_userid."%'" : null;
                  $searchStr .= $search_comtype ? " AND company_type LIKE '%".$search_comtype."%'" : null;
                  $searchStr .= $search_job ? " AND data_type LIKE '%".$search_job."%'" : null;
                  $searchStr .= $search_addr2 ? " AND address LIKE '%".$search_addr2."%'" : null;
                  $searchStr .= $search_addr1 ? " AND address1 LIKE '%".$search_addr1."%'" : null;
                    ...
                    while($row = mysqli_fetch_array($res1)) {
                      ...
                      if($row_mem_reg[mem_code] != ''){
                        ...
                        $link = "card_list.php?mem_id=".$row_mem_reg['mem_id'];
                          ...
                          $edit_type = "get_cust_edit";
                          $idx = $row['seq'];
                          $user_id = $row['user_id']?$row['user_id']:'';
                          $name = $row['ceo']?$row['ceo']:'';
                          $phone1 = $row['cell']?$row['cell']:'';
                          $phone2 = $row['cell1']?$row['cell1']:'';
                          $email = $row['email']?$row['email']:'';
                          $birthday = $row['birthday']?$row['birthday']:'';
                          $work_type = $row['company_type']?$row['company_type']:'';
                          $company_name = $row['company_name']?$row['company_name']:'';
                          $job = $row['data_type']?$row['data_type']:'';
                          $company_addr = $row['address']?$row['address']:'';
                          $home_addr = $row['address1']?$row['address1']:'';
                          $link = $card_url = $row['url']?$row['url']:'';
                          $memo = $row['memo']?$row['memo']:'';
                          $reg_date = $row['regdate']?$row['regdate']:'';
                      }
                  ?>
                      <tr>
                        <td><input type="checkbox" class="check" id="check_one_member" name="" value="<?=$row['seq']?>"><?=$number--?></td>
                        ...
                        <td> <a href="customer_info_detail.php?idx=<?=$row['seq']?>&edit_type=<?=$edit_type?>&mem_code=<?=$mem_code?>&pre_page=get_list">수정</a>/<a href="javascript:del_member_info('<?=$row['seq']?>')">삭제</a>/<a href="javascript:move_reg_list('<?=$row['seq']?>', 'get_cust')">이동</a> </td>
                      </tr>
                    <?
                    $c++;
                    $i++;
                    ...
                    ?>
                        <tr>
                            <td colspan="16" style="text-align:center;background:#fff">
                    	        등록된 내용이 없습니다.
                            </td>
                        </tr>
                    <?
...
function del_member_info(idx){
  ...
  if(msg){
      $.ajax({
        type:"POST",
        url:"/admin/ajax/manage_customers.php",
        data:{id:idx, mode:'del', delete_name:"get_cust_list"},
        success:function(){
          alert('삭제되었습니다.');
          location.reload();
        },
