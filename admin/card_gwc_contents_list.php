        ...
        <form method="get" name="search_form" id="search_form">
          <div class="box-tools">
            <div class="input-group" style="display:flex;width:350px">
              ...
              <? if ($domain == "kiam") { ?>
                <input type="text" name="selling" id="selling" class="form-control input-sm pull-right" placeholder="셀링소속" value="<?= $selling ?>">
                <input type="text" name="iam" id="iam" class="form-control input-sm pull-right" placeholder="아이엠소속" value="<?= $iam ?>">
              <? } ?>
          ...
          <div class="box-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                $searchStr = "contents.gwc_con_state!= 0 ";
                if($domain != "kiam")
                  $searchStr .= " AND (mem.site_iam = '{$domain}' or mem.site='{$domain}')";
                ...
                $searchStr .= $selling ? " AND mem.site = '{$selling}' " : null;
                $searchStr .= $iam ? " AND mem.site_iam = '{$iam}' " : null;
                ...
                if ($searchStr == null) {
                  $count_query = "select count(idx) from Gn_Iam_Contents_Gwc contents where contents.gwc_con_state!=0";
                } else {
                  if($selling || $iam)
                    $count_query = "select count(idx) from Gn_Iam_Contents_Gwc contents inner join Gn_Member mem on mem.mem_id=contents.mem_id where contents.gwc_con_state!=0 $searchStr";
                  else
                    $count_query = "select count(idx) from Gn_Iam_Contents_Gwc contents where contents.gwc_con_state!=0 $searchStr";
                }
                ...
                $query = "SELECT  contents.idx, contents.mem_id,contents.contents_type,contents.contents_img,contents.group_id," .
                  ...
                  "FROM Gn_Iam_Contents_Gwc contents WHERE $searchStr";
                if($selling || $iam || $domain != "kiam"){
                  $query = "SELECT mem.site, mem.site_iam, mem.gwc_provider_name, mem.mem_name, mem.gwc_manage_price, mem.mem_code, contents.idx, contents.mem_id,contents.contents_type,contents.contents_img,contents.group_id," .
                  "contents.contents_title,contents.contents_url,contents.card_short_url,contents.westory_card_url,contents.contents_sell_price," .
                  "contents.contents_temp,contents.contents_like,contents.contents_share_text,contents.contents_share_count,contents.up_data,contents.sample_display,contents.sample_order,contents.public_display,contents.gwc_con_state,contents.contents_price,contents.product_seperate,contents.send_provide_price,contents.prod_manufact_price,contents.card_idx,contents.prod_sehu_price " .
                  "FROM Gn_Iam_Contents_Gwc contents inner join Gn_Member mem on mem.mem_id = contents.mem_id WHERE $searchStr";
                }
                ...
                while ($row = mysqli_fetch_assoc($res)) {
                  if(!$selling && !$iam && $domain == "kiam"){
                    ...
                    $row = array_merge($row,$mem_row);
                  }
                  if ($row['gwc_manage_price']) {
                    $mng_price = $row['gwc_manage_price'];
                  } else {
                  ...
                  <tr>
                    ...
                    <td>
                      <div style="overflow-x:hidden;width:100px;">
                        <?= $row['site'] ?>/<br><?= $row['site_iam'] ?>
                      </div>
                    </td>
                    <td>
                      <div style="overflow-x:hidden;width:100px;">
                        <?= $row['mem_name'] ?>/<br><?= $row['mem_id'] ?>
                      </div>
                    </td>
                    ...
                    <td><input type="number" name="manage_price_<?= $row['mem_code'] ?>" value="<?= $mng_price ?>" style="width:65px;font-size: 11px;"><button onclick="save_manage_price('<?= $row['mem_code'] ?>');return false;" style="font-size: 11px;color: white;background-color: black;padding: 0px 5px;">저장</button></td>
                    <td><?= $row['up_data'] ?></td>
