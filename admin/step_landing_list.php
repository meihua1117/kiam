      ...
      <div class="box-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            $query = "SELECT count(a.m_id) FROM Gn_landing a WHERE 1=1 $searchStr";
            $res      = mysqli_query($self_con, $query);
            //$totalCnt	=  mysqli_num_rows($res);	
            $totalRow  =  mysqli_fetch_array($res);
            $totalCnt = $totalRow[0];

            $query = "SELECT a.m_id, a.title, a.description, a.read_cnt, a.status_yn,a.short_url, a.cnt, a.regdate, a.landing_idx 
                            FROM Gn_landing a WHERE 1=1 $searchStr";
            $limitStr       = " LIMIT " . (($startPage - 1) * $pageCnt) . ", " . $pageCnt;
            $number      = $totalCnt - ($nowPage - 1) * $pageCnt;
            $orderQuery .= " ORDER BY a.landing_idx DESC $limitStr";
