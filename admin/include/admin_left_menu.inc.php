...
if ($HTTP_HOST == "kiam.kr")
    ...
    $host = $HTTP_HOST;
$query = "select shoping_manage from Gn_Service where sub_domain like '%{$host}%'";
$res = mysqli_query($self_con, $query);
$domainData = mysqli_fetch_assoc($res);
if ($HTTP_HOST != "kiam.kr") {
    $shoping_manage = $domainData['shoping_manage'];
} else {
    $shoping_manage = 1;
}
                        ...
                        <? if ($_SESSION['one_member_admin_id'] != "onlyonemaket") { ?>
                            <li <?= $fileName == "ad_list.php" || $fileName == "ad_detail.php" ? " class='active'" : "" ?>><a href="/admin/ad_list.php"><i class="fa fa-book"></i> <span>플랫폼광고관리</span></a>
                                <ul class="treeview-menu" style="display: block;">
                                    ...
                                    <li <?= $fileName == "ad_store_list.php" || $fileName == "ad_store_detail.php" ? " class='active'" : "" ?>>
                                        <a href="/admin/ad_store_list.php"><i class="fa fa-circle-o"></i> <span>스토어광고관리</span></a>
                                    </li>
                                </ul>
                            </li>
                        <? } ?>
