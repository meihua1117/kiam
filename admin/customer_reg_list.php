...
function deleteMultiRow() {
	...
	if(confirm('삭제하시겠습니까?')) {
      ...
      dataType:"json",
      data:{mode:'del', delete_name:"reg_cust_list", id:no_array.toString()},
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            등록고객 정보관리
            <small>등록고객의 정보를 관리합니다.</small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">등록고객 정보관리</li>
          </ol>
        </section>
        ...
        <section class="content">
          <div class="row" id="toolbox">
            <div class="col-xs-12" style="padding-bottom:20px">
              ...
              <button class="btn btn-primary pull-right" style="margin-right: 5px;border: none;" onclick="location='customer_reg_list.php';return false;">등록고객 정보관리</button>
          ...
          <div class="row">
              ...
              <div class="box">
                <div class="box-body">
                  ...
                  // 검색 조건을 적용한다.
                  $searchStr .= $search_userid ? " AND mem_id LIKE '%".$search_userid."%'" : null;
                  $searchStr .= $search_comtype ? " AND work_type LIKE '%".$search_comtype."%'" : null;
                  $searchStr .= $search_job ? " AND job LIKE '%".$search_job."%'" : null;
                  $searchStr .= $search_addr2 ? " AND company_addr LIKE '%".$search_addr2."%'" : null;
                  $searchStr .= $search_addr1 ? " AND home_addr LIKE '%".$search_addr1."%'" : null;
                    ...
                    while($row = mysql_fetch_array($res)) {                       	
                      ...
                      if($row_mem_reg[mem_code] != ''){
                        ...
                        $link = "card_list.php?mem_id=".$row_mem_reg['mem_id'];
                          ...
                          $edit_type = "reg_cust_edit";
                          $idx = $row['id'];
                          $user_id = $row['mem_id']?$row['mem_id']:'';
                          ...
                          $job = $row['job']?$row['job']:'';
                          $company_addr = $row['company_addr']?$row['company_addr']:'';
                          $home_addr = $row['home_addr']?$row['home_addr']:'';
                          $link = $card_url = $row['link']?$row['link']:'';
                          $memo = $row['memo']?$row['memo']:'';
                          $reg_date = $row['reg_date']?$row['reg_date']:'';
                      }
                  ?>
                      <tr>
                        <td><input type="checkbox" class="check" id="check_one_member" name="" value="<?=$row['id']?>"><?=$number--?></td>
                        ...
                        <td> <a href="customer_info_detail.php?idx=<?=$row['id']?>&edit_type=<?=$edit_type?>&mem_code=<?=$mem_code?>&pre_page=reg_list">수정</a>/<a href="javascript:del_member_info('<?=$row['id']?>')">삭제</a> </td>
                      </tr>
                    <?
                    $c++;
                    $i++;
...
function del_member_info(idx){
	...
	if(msg){
		...
		type:"POST",
		url:"/admin/ajax/manage_customers.php",
		data:{id:idx, mode:'del', delete_name:"reg_cust_list"},
		success:function(){
			alert('삭제되었습니다.');
			location.reload();
		},
