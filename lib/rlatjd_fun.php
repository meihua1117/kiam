...
function message_send($from, $to, $subject, $content, $return_msg)//현재 이용하지 않음
{
    /******************** 인증정보 ********************/
...
function gcUpload($file_name, $file_tmp_name, $file_size, $folder, $filename) {
    ...
    move_uploaded_file($file_tmp_name,$_SERVER['DOCUMENT_ROOT']."/$folder/$file_name") or die($file_tmp_name.'을 '.$_SERVER['DOCUMENT_ROOT']."/$folder/$file_name"." Upload에 실패했습니다.");
