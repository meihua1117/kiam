...
//		echo '<br />';
	...
	private function _readDefinedName()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			// note: there can also be additional data, this is not include_onced in $flen
...
//		echo '<br />';
	...
	private function _include_onceCellRangeFiltered($cellRangeAddress)
	{
		$include_onceCellRange = true;
		if ($this->getReadFilter() !== NULL) {
			$include_onceCellRange = false;
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
				if ((strpos($cellRangeAddress,':') !== FALSE) &&
					($this->_include_onceCellRangeFiltered($cellRangeAddress))) {
