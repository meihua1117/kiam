    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            <?  }
                ...
                $.ajax({
                    ...
                    data:{point_use:true, service_id:'<?=$row_point_state[mem_id]?>'},
        ...
        function recvPhoneStatus(status){
            if(status == "N"){
                ...
                if(getCookie('show_alarm_phonestate') == ''){ AppScript.getPhonePos(); }
            }
        }
        ...
        function recvPhonePos_show(lat, lng, idx, map_pos){
            ...
            $.ajax({
                ...
                data:{type:"distance_calc", start_info:start_info, end_info:map_pos},
                dataType: 'text',
                success:function(data){
        ...
        function displayMall(lim, off) {
            ...
            $.ajax({
                ...
                data: "limit=" + lim + "&offset=" + off + "&search_key=<?=$_GET['search_key']?>" + "&cur_win=<?=$_GET['cur_win']?>" + "&sort=<?=$_GET['sort']?>"
                    + "&mall_type=<?=$_GET[mall_type]?>" + "&mall_type1=<?=$_GET['mall_type1']?>" + "&contents_mode=" + cont_mode + "&member_iam=<?=$member_iam['site_iam']?>" ,
                cache: false,
                success: function(html) {
        ...
        function displayContents(lim, off) {
           $(".contents_per_page").load("ajax/get_iam_contents.php?" + "limit=" + lim + "&offset=" + off + "&search_key=<?=$_GET['search_key']?>" + "&cur_win=<?=$cur_win?>"  + "&card_owner=<?=$card_owner?>"
                    + "&name_card=<?=$_GET[name_card]?>" + "&key1=<?=$_GET['key1']?>" + "&key2=<?=$_GET['key2']?>"+ "&sort=<?=$_GET['sort']?>"+ "&request_uri=<?=$_SERVER['REQUEST_URI']?>", function(responseTxt, statusTxt, xhr){
        ...
        $(window).scroll(function() {
            <? if($_GET['cur_win'] == "best_sample" || $_GET['cur_win'] == "sample" || $_GET['cur_win'] == "recent_sample" ){?>
                if ($(window).scrollTop() + $(window).height() > $(".sample_main").height() && !busy) {
                    busy = true;
                    offset = limit + offset;
                    setTimeout(function() { displaySample(limit, offset); }, 500);
                }
            <?} else if($_GET['cur_win'] == "iam_mall"){?>  
                if ($(window).scrollTop() + $(window).height() > $(".sample_main").height() && !busy) {
                    busy = true;
                    offset = limit + offset;
                    setTimeout(function() { displayMall(limit, offset); }, 500);
                }              
            <?}?>          
        });
    ...
    <header id="header" style="position: fixed; z-index: 100; width:100%;max-width: 768px;">
                    ...
                    <div class="inner-wrap" style = "text-align:center;padding : 0px">
                        ...
                        <?
                            if(!$_SESSION[site_iam])
                                ...
                                $my_iam_link = "javascript:go_my_con_page('".$_SESSION[sess_mem_id]."','".$_SESSION[site_iam]."')";
                        ?>
            ...
            <div class="panel-group">
                ...
                <div style="background-color:white;padding: 10px;" class = "mall-group">
                    <ul class="nav nav-tabs mall-nav-tabs">
                        <li class="nav-item <?if($mall_type1 == 'all') echo 'active'?>" style="border: 1px solid #ddd;width:20%;">
                            <a class="nav-link" style="padding:5px" href="<?='?cur_win='.$cur_win.'&mall_type='.$mall_type.'&mall_type1=all'?>">전체</a>
                        </li>
                        <li class="nav-item  <?if($mall_type1 == 'msg_set') echo 'active'?>" style="border: 1px solid #ddd;width:20%;">
                            <a class="nav-link" style="padding:5px" href="<?='?cur_win='.$cur_win.'&mall_type='.$mall_type.'&mall_type1=msg_set'?>">메시지</a>
                        </li>
                        <li class="nav-item <?if($mall_type1 == 'card') echo 'active'?>" style="border: 1px solid #ddd;width:20%;">
                            <a class="nav-link" style="padding:5px" href="<?='?cur_win='.$cur_win.'&mall_type='.$mall_type.'&mall_type1=card'?>">재능마켓</a>
                        </li>
                        <li class="nav-item <?if($mall_type1 == 'iam') echo 'active'?>" style="border: 1px solid #ddd;width:20%;">
                        ...
                        <li class="nav-item <?if($mall_type1 == 'service') echo 'active'?>" style="border: 1px solid #ddd;width:20%;">
                            <a class="nav-link" style="padding:5px" href="<?='?cur_win='.$cur_win.'&mall_type='.$mall_type.'&mall_type1=service'?>">중고마켓</a>
                        </li>
                    </ul>
                    <div class="panel panel-default" style="display: flex;border-color: #ddd;margin-left:auto;margin-right:auto;justify-content: space-between;">
        ...
        <?}else if($cur_win == "we_story"){?>
            <div class="panel-group" id="we_story_tab">
                <div class="recom_like_tab" style="<?if($_GET[key1] != 3 && $_GET[key1] != 4) echo 'column-count:1;background-color: #f5f5f5;padding:0px;'; if($_GET[key1] == 4) echo 'padding:5px 0px;'?>">
                    <?if($_GET[key1] == 4){?>
    ...
    <main id="star" class="common-wrap" style="margin-top: <?= $star_top.'px'?>" >
            ...
            <section id="middle"  >
                ...
                <ul class="nav nav-tabs" id="myTab" role="tablist" >
                    <li class="nav-item profile-tab tab_1 active in">
                        <a class="nav-link" id="profile-tab"  title="<?=$MENU['IAM_TAB']['T1_TITLE'];?>" data-toggle="tab" href="#profile" role="tab"
                        aria-controls="profile" aria-selected="true"><b><?=$MENU['IAM_TAB']['T1'];?></b><br><small id="my_profile_count"></small></a>
                    </li>
                    <li class="nav-item paper-tab tab_1" onclick="loginCheckShowModal()">
                        <a class="nav-link" id="paper-tab" title="<?=$MENU['IAM_TAB']['T5_TITLE'];?>" data-toggle="tab" href="#paper" role="tab" onclick="javascript:getIamPaper('<?=$card_owner?>', '<?=$card_master?>', '<?=$search_range?>', '<?=$phone_count?>', 1 );"
                        aria-controls="paper" aria-selected="false"><b><?=$MENU['IAM_TAB']['T5'];?></b><small><br>(<?=number_format($paper_total_count)?>)</small></a>
                    </li>
                    <li class="nav-item contact-tab tab_1" onclick="loginCheckShowModal()">
                        <a class="nav-link" id="contact-tab" title="<?=$MENU['IAM_TAB']['T3_TITLE'];?>" data-toggle="tab" href="#contact" role="tab" onclick="javascript:getIamContact('<?=$card_owner?>', '<?=$card_master?>', '<?=$search_range?>', '<?=$phone_count?>', 1, 0);"
                        aria-controls="contact"
                        aria-selected="false"><b><?=$MENU['IAM_TAB']['T3'];?></b><small><br>(<?=number_format($contact_total_count)?>)</small></a>
                    </li>
                    <li class="nav-item friends-tab tab_1"  onclick="loginCheckShowModalFriends()">
                        <a class="nav-link" id="friends-tab" title="<?=$MENU['IAM_TAB']['T4_TITLE'];?>" data-toggle="tab" href="#friends" role="tab" onclick="javascript:getIamFriends('<?=$card_owner?>', '<?=$card_master?>', '<?=$phone_count?>', '', 1 );"
                        aria-controls="friends"
                        aria-selected="false"><b><?=$MENU['IAM_TAB']['T4'];?></b><small><br>(<?=number_format($card_count_row[0])?>)</small></a>
                    </li>
                </ul>
                                            ...
                                            <div class="color-chips">
                                                ...
                                                if($cur_win == "my_info" &&
                                                    (($user_mem_code == $card_owner_code && !$_SESSION[iam_member_subadmin_id] && (!$pay_status || $domainData[service_type] == 2)) ||
                                                        ($user_mem_code != $card_owner_code && !$share_sub_admin && (!$share_pay_status || $domainData[service_type] == 2)))){//무료회원이거나 단체회원이면 분양사관리자 1번카드를 자신의 1번카드로 한다
                                                    //$share_pay_status = false;
                                                    ...
                                                    ?>
                                                    <a href="?<?=$first_card_url.$card_owner_code?>" onclick="show_next_iam('<?=$title_row[next_iam_link]?>');" class="J_card_num <?=$private_class?> <?=$active_class?>" id = "card_title"
                                                    style="display: <?=$title_display?>;"
                                                    title= <?=$title_row[card_title]?>><?=$title_row[card_title]?$title_row[card_title]:$card_count+1?><?=$title_row[next_iam_link]==""?"":"+"?><?=$n_cont_row[0] > 0 ? "(".$n_cont_row[0].")" : ""?></a>
                                                    <?
                                                    $card_count++;
                                                ...
                                                while($black_circle_row=mysql_fetch_array($black_circle_result)) {
                                                    ...
                                                    if($private_class != "private" || $_SESSION[iam_member_id] == $black_circle_row[mem_id] || (isset($_GET['smode']) && $card_idx == $_GET['slink'])) {
                                                        ...
                                                        $cont_count += $cont_row[0];
                                                        ?>
                                                        <a href="?<?=$card_link.$card_owner_code.(isset($_GET['smode'])?'&smode='.$_GET['smode'].'&slink='.$_GET['slink'].'&smsg='.$_GET['smsg']:'')?>" class=" J_card_num <?=$private_class?> <?=$active_class?>" id = "card_title" onclick="show_next_iam('<?=$black_circle_row[next_iam_link]?>');"
                                                        draggable="false" ondragstart="drag(event,'<?=$card_link?>')" ondrop="drop(event,'<?=$card_link?>')" ondragover="allowDrop(event)"
                                                        ontouchstart="pickup(event,'<?=$card_link?>')" ontouchmove="move(event)" ontouchend = "pickdown(event)"
                                                        style="z-index:10;display: <?=$title_display?>;"
                                                        title= <?=$black_circle_row[card_title]?>>
                                                ...
                                                if($_SESSION[iam_member_id] && $card_owner_code == $user_mem_code){
                                                    if($pay_status){?>
                                                        <a onclick="show_create_card_popup();" class="J_card_num" id = "card_title" title= "<?=$MENU['IAM_CARD_M']['CM9_TITLE'];?>"
                                                        style="z-index:10;background:white;color:#529c03;display: <?=$title_display?>">
                                                        <span style="font-size:15px">새 카드 만들기</span>
                                                        </a>
                                                    <?}else{?>
                                                        <a onclick="show_pay_popup('<?= $domainData[service_type] ?>');" class="J_card_num" id = "card_title" title= "<?=$MENU['IAM_CARD_M']['CM9_TITLE'];?>"
                                                        style="z-index:10;background:white;color:#529c03;display: <?=$title_display?>">
                                                        <span style="font-size:15px">새 카드 만들기</span>
                                                        </a>
                                                    <?}?>
                                        ...
                                        <div class="formBox">
                                            ...
                                            <div style = "display:none;margin-top:-30px">
                                                ...
                                                <div class="J_card_email" style="font-size:14px;margin-top:1px;">
                                                    ...
                                                    <?}?>
                                                    <div style="margin-top:25px;vertical-align:middle;display: flex;">
                                                        <input id="edit_card_online1_text" class="card-input" 
                                                            value="<?=$online1_check == "Y"?$story_online1_text:""?>" placeholder="사이트이름">
                                                        <input id="edit_card_online1" class="card-input" style="margin-left:5px;" 
                                                            value="<?=$online1_check == "Y"?$story_online1:""?>" placeholder="사이트주소">
                                                    ...
                                                    <div style="margin-top:1px;vertical-align:middle;display: flex;">
                                                        <input id="edit_card_online2_text" class="card-input" 
                                                            value="<?=$online2_check == "Y"?$story_online2_text:""?>" placeholder="사이트이름">
                                                        <input id="edit_card_online2" class="card-input" style="margin-left:5px;" 
                                                            value="<?=$online2_check == "Y"?$story_online2:""?>" placeholder="사이트주소">
                                                ...
                                                <div id="edit_card_special" style="margin-bottom:0px;display:none">
                                                    <textarea id="edit_card_keyword" class="input card-input" name="edit_card_keyword" style="text-align:left;width:100%;"
                                                        placeholder="아이엠에서 나를 검색할수 있는 단어 (30개 이내)로 입력하고, 입력시 [,]으로 구분하세요. (예시 : 강사,마케터,변호사,대안학교,노래방, 공부방 등)"><?=$cur_card[card_keyword]?></textarea>
                                                    <input id="edit_card_next_iam_link" class="J_card_address card-input"  
                                                        value='<?=$cur_card[next_iam_link]?>' placeholder="카드제목 클릭시 새창으로 열릴 다른 IAM주소 입력">
                                                </div>
                            ...
                            <div class="search-box clearfix J1">
                                <?if($_SESSION[iam_member_id] == $card_owner && $_SESSION[iam_member_id] == $card_master) {?>
                                    <div class="row" style="margin-left: 8px;margin-bottom: 10px;margin-top: 7px;">
                                        <div class="left">
                                            <div class="buttons">
                                                <a href="javascript:contact_range('1')" class="button"
                                                <?if((int)$search_range==1) {?>style="background-color: #000;"
                                                <?}else{?>style="background-color: #fff;color:black;"
                                                    <?}?>>가</a>
                                                <a href="javascript:contact_range('3')" class="button"
                                                <?if((int)$search_range==3) {?>style="background-color: #000;"
                                                <?}else{?>style="background-color: #fff;color:black;"
                                                    <?}?>><i class="fa fa-history" aria-hidden="true"></i></a>
                                                <input type="hidden" name="search_range" id="search_range"
                                                    value="<?=$search_range?>">
                                            </div>
                            ...
                            <div class="search-box clearfix J2" id="friends_search">
                                <?if($_SESSION[iam_member_id] == $card_owner && $_SESSION[iam_member_id] == $card_master) {?>
                                    <div class="row" style='margin-left: 8px;margin-bottom: 10px;margin-top: 7px;'>
                                        <div class="left">
                                            <div class="buttons">
                                                <a href="javascript:friends_range('1');" class="button"
                                                <?if((int)$search_range2==1) {?>style="background-color: #000;"
                                                <?}else{?>style="background-color: #fff;color:black;"
                                                    <?}?>>가</a>
                                                <a href="javascript:friends_range('3');" class="button"
                                                <?if((int)$search_range2==3) {?>style="background-color: #000;"
                                                <?}else{?>style="background-color: #fff;color:black;"
                                                    <?}?>><i class="fa fa-history" aria-hidden="true"></i></a>
                                        ...
                                        <div class="right">
                                            <select name="search_type" id="search_type" style="font-size:12px;margin-right:20px; border: 1px solid #ddd;background: #fff;border-radius: 5px;" onchange="friends_submit('<?=$show_all?>')">
                                                ...
                                                <option value="2" <?if((int)$search_type==2){?>selected
                                                    <?}?>>공개 프로필</option>
                                                <option value="3" <?if((int)$search_type==3){?>selected
                                                    <?}?>>소속 회원</option>
                                                <option value="4" <?if((int)$search_type==4){?>selected
                                                    <?}?>>조인 회원</option>
                                            </select>
                                        </div>
                                    </div>
        ...
        ?>
            ...
            <?if($gkind == "recommend"){
                ...
                ?>
                    <div class="group_sample_content-item" id="group_sample_contents_list"
                        style="margin-bottom: 20px;box-shadow: 2px 3px 3px 1px #eee;border: 1px solid #ccc;">
            ...
            <?}else if($gkind == "search"){?>
                    ...
                    ?>
                            <div style="padding-top: 10px;">
                                <div style="border:1px solid #ddd;background-color: #ffffff;border-top-right-radius:10px;border-top-left-radius:10px;;padding-bottom: 2px;position:relative;padding-bottom: 60%;overflow: hidden;">
                                    <div class="content" style="background-image:url('<?=$group_row[main_img1]?>');background-size: cover;height:100%;background-position: center;" 
                                        onclick = "location.href = '?' + '<?=$group_row[card_short_url].$row_mem_info[mem_code]?>'+'&cur_win=group-con&gkind=' +'<?=$group_row[group_id]?>'">
                                    </div>
                                </div>
                ...
                <div style="width:100%;border:1px solid #ddd;padding:5px;margin-top:5px">
                    ...
                    ?>
                            <div style="padding-top: 10px;">
                                <div style="border:1px solid #ddd;background-color: #ffffff;border-top-right-radius:10px;border-top-left-radius:10px;;padding-bottom: 2px;position:relative;padding-bottom: 60%;overflow: hidden;">
                                    <div class="content" style="background-image:url('<?=$row_card_g[main_img1]?>');background-size: cover;height:100%;background-position: center;" 
                                        onclick = "location.href = '?' + '<?=$row_card_g[card_short_url].$row_mem_g[mem_code]?>'+'&cur_win=group-con&gkind=' +'<?=$group_row[group_id]?>'">
                                    </div>
                                </div>
                                <div style="border:1px solid #ddd;border-bottom-left-radius: 10px;border-bottom-right-radius: 10px;">
                                    <h4 style="margin-left:10px;margin-top:10px"><?=$group_row[name]?></h4>
                                        ...
                                        <div class="color-chips">
                                            ...
                                            while($black_circle_row=mysql_fetch_array($black_circle_result)) {
                                                ...
                                                <a href="?<?=$card_link?>" class="J_card_num <?=$private_class?> <?=$active_class?>" id = "card_title" onclick="show_next_iam('<?=$next_iam_link?>');"
                                                    draggable="false" ondragstart="drag(event,'<?=$card_link?>')" ondrop="drop(event,'<?=$card_link?>')" ondragover="allowDrop(event)"
                                                    ontouchstart="pickup(event,'<?=$card_link?>')" ontouchmove="move(event)" ontouchend = "pickdown(event)"
                                                    style="z-index:10;display: <?=$title_display?>;font-size: 15px;"
                                                    title= <?=$black_circle_row[card_title]?>>
                                            ...
                                            if($_SESSION[iam_member_id] == $group_manager){?>
                                                <a onclick="show_create_group_card_popup();" class="J_card_num" id = "card_title" title= "<?=$MENU['IAM_CARD_M']['CM9_TITLE'];?>"
                                                    style="font-size: 15px;font-weight: 700;text-align: center;z-index:10;background:white;color:#529c03;display: <?=$title_display?>">
                                                        <span style="font-size:15px">새 카드 만들기</span>
                                                    </a>
                                                <?}?>
                                            <script>
        ...
        <section id="bottom">
            ...
            <?}else if($cur_win == "unread_notice" && $notice_count != 0){?>
                ...
                while($notice_row = mysql_fetch_array($result_notice)){
                    ...
                    $href = "/?" . $row_card_info['card_short_url'] . $row_sender_info['mem_code']."&site=".$_SESSION[site_iam]."&sess_mem_id=".$_SESSION[sess_mem_id];
                    ...
                    <div class="content_area">
                        ...
                        ?>
                            <div class="user-item" style="<?=$user_display . $pad_top ?>">
                                <a href="/?<?=strip_tags($contents_row['westory_card_url'].$m_code.'&site='.$_SESSION[site_iam]).'&sess_mem_id='.$_SESSION[sess_mem_id]?>" class="img-box"  target = "_blank">
                                ...
                                <div class="wrap image_mode">
                                    <a href="/?<?=strip_tags($contents_row['westory_card_url'].$m_code.'&site='.$_SESSION[site_iam]).'&sess_mem_id='.$_SESSION[sess_mem_id]?>" class="user-name" style="">
                                        <?=$contents_user_name ?>
                                    </a>
                                    <?if($_GET[iamstore] != 'N' || $_GET[cate_prod] == '1268514'){?>
                                    <a href="/?<?=strip_tags($contents_row['westory_card_url'].$m_code.'&site='.$_SESSION[site_iam]).'&sess_mem_id='.$_SESSION[sess_mem_id]?>" class="date" style="">
                                ...
                                <div class="wrap pin_mode" style="display:none;">
                                    <?//if($contents_row['contents_title'] != "") { ?>
                                        <!-- <a href="/?<?=strip_tags($contents_row['westory_card_url'].$m_code.'&site='.$_SESSION[site_iam]).'&sess_mem_id='.$_SESSION[sess_mem_id]?>" class="user-name pin-mode" style="text-overflow: ellipsis;white-space: nowrap;width:100%">
                                ...
                                <div class="desc" >
                                    ...
                                    <?if($content_images == null){
                                        if(!$_SESSION[iam_member_id] || $cur_card[share_send_card] != 0 || ($cur_win != "shared_receive" && $_SESSION[iam_member_id] != $contents_row['mem_id'] && $_SESSION[iam_member_id] != $group_manager)){?>
                                            <a class="content-utils" style="cursor:pointer;position:absolute;bottom:5px;right:10px;z-index:10;" 
                                        ...
                                        <?} else{?>
                                            <a onclick="show_contents_utils(<?=$contents_row['idx']?>);" class="content-utils"
                                                style="position: absolute;bottom:5px;right: 10px;z-index:10;" title="">
                            ...
                            <div class="media-wrap" >
                                ...
                                <div class="media-inner <?=$_GET[type]?>" style="overflow-y: hidden;<?if($contents_row['contents_type'] == 1 && count($content_images) == 0) echo "min-height :30px;";?>">
                                    <?if($content_images != null){
                                        if(!$_SESSION[iam_member_id] || $cur_card[share_send_card] != 0 || ($cur_win != "shared_receive" && $_SESSION[iam_member_id] != $contents_row['mem_id'] && $_SESSION[iam_member_id] != $group_manager)){?>
                                            <a class="content-utils" style="cursor:pointer;position:absolute;top:5px;right:10px;z-index:10;" 
                                                onclick="showSNSModal_byContents('<?=$contents_row['idx']?>', '<?=$contents_row['gwc_con_state']?>');">
                                                <img src="/iam/img/menu/icon_share_white.png"  style="width:13px;height:16px;margin-left:0px;margin-top:-1px;">
                                            </a>
                                        <?} else{?>
                                            <a onclick="show_contents_utils(<?=$contents_row['idx']?>);" class="content-utils"
                                                style="position: absolute;top:5px;right: 10px;z-index:10;" title="">
                                                <!--i class="fa fa-pencil-square-o" aria-hidden="false"></i-->
                                                <img src = "/iam/img/menu/icon_utils_box.png" style="height:20px;">
                                            </a>
                                    ...
                                    if((int)$contents_row['contents_type'] == 1 || (int)$contents_row['contents_type'] == 3 ) {
                                        if(count($content_images) > 1 && $contents_row['landing_mode'] == "N" && !$contents_row['gwc_con_state']){?>
                                            <button onclick="show_all_content_images('<?=$contents_row[idx]?>')"  id = "content_all_image<?=$contents_row[idx]?>"
                                                    style="position: absolute;right:0px;bottom:0px;font-size: 14px;opacity: 60%;background: black;color: white;">
        ...
        <div class="pagination">
            <ul <? if($_GET['cur_win'] == "best_sample" || $_GET['cur_win'] == "sample" || $_GET['cur_win'] == "recent_sample" ){ echo 'hidden';}?>>
...
<script>
    ...
    function save_order_option(){
        ...
        if(page_type == 'pay'){
            ...
            $.ajax({
                ...
                dataType:"json",
                data:{cont_idx:cont_idx, conts_cnt:conts_cnt, conts_price:conts_price, salary:salary, seller_id:seller_id, option:str, mode:'save_order_cart', gwc_order_option_content : JSON.stringify(gwc_order_option_array)},
                success:function(data){
    ...
    function save_seller_mem_leb(){
        ...
        $.ajax({
            ...
            url:"/iam/ajax/manage_request_list.php",
            dataType:"json",
            data:{mode:'req_seller_leb_up', seller_mem_id:seller_mem_id, seller_mem_phone:seller_mem_phone, seller_mem_addr:seller_mem_addr, seller_mem_bank_name:seller_mem_bank_name, seller_mem_bank_owner:seller_mem_bank_owner, seller_mem_bank_account:seller_mem_bank_account, seller_mem_email:seller_mem_email},
            success:function(data){
    ...
    function show_order_option(con_idx, type, gwc_state){
        ...
        if(mem_id == ''){
            ...
            $.ajax({
                ...
                data:{mode:'check_gwc_member', mem_id:'<?=$_SESSION[iam_member_id]?>'},
                dataType:'json',
                success:function(data){
                    if(data.res == "0"){
        ...
        if(gwc_state == 0){
            ...
            $.ajax({
                ...
                url:"/iam/ajax/manage_request_list.php",
                dataType:"json",
                data:{con_idx:con_idx, mode:'show_conts_price'},
                success:function(data){
            ...
            $.ajax({
                ...
                url:"/iam/ajax/manage_request_list.php",
                dataType:"html",
                data:{con_idx:con_idx, mode:'show_order_data'},
                success:function(data){
    ...
    function save_req_mem_leb(){
        ...
        $.ajax({
            ...
            url:"/iam/ajax/manage_request_list.php",
            dataType:"json",
            data:{mode:'req_mem_leb_up', req_mem_id:req_mem_id, req_mem_phone:req_mem_phone, req_mem_addr:req_mem_addr, req_mem_bank_name:req_mem_bank_name, req_mem_bank_owner:req_mem_bank_owner, req_mem_bank_account:req_mem_bank_account, req_mem_email:req_mem_email},
            success:function(data){
    ...
    function save_paper(){
        ...
        $.ajax({
            ...
            url:"/iam/ajax/manage_paper_list.php",
            dataType:"html",
            data:{save_data:'Y', paper_seq:paper_seq, paper_name:paper_name, paper_job:paper_job, paper_org_name:paper_org_name, paper_address:paper_address, paper_phone1:paper_phone1, paper_phone2:paper_phone2, paper_mobile:paper_mobile, paper_fax:paper_fax, paper_email1:paper_email1, paper_email2:paper_email2, paper_memo:paper_memo},
            success:function(data){
                alert("저장되었습니다.");
                $("#paper_list_edit_modal").modal("hide");
            }
    ...
    function cancel_req(req_idx){
        if(confirm('취소하시겠습니까?')){
            $.ajax({
                ...
                url:"/iam/ajax/manage_request_list.php",
                dataType:"json",
                data:{req_idx:req_idx, req_state:'N', mode:'update_state'},
                success:function(data){
                    location.reload();
                }
            })
    ...
    function allow_req(req_idx){
        if(confirm('신청하시겠습니까?')){
            $.ajax({
                ...
                url:"/iam/ajax/manage_request_list.php",
                dataType:"json",
                data:{req_idx:req_idx, req_state:'Y', mode:'update_state'},
                success:function(data){
                    location.reload();
                }
            })
    ...
    function showCardState(val1, val){
        $.ajax({
            ...
            dataType:"json",
            data:{id:'<?=$_SESSION[iam_member_id]?>', state:val, card_show:true, short_url:val1},
            success:function(data){
                location.reload();
            }
        })
    ...
    function show_map_address(card_idx){
        $.ajax({
            type:"POST",
            url:"/iam/ajax/show_naver_map.php",
            data:{type:"address", card_idx:card_idx},
            dataType: 'text',
            success:function(data){
    ...
    function show_map_distance(card_idx){
        $.ajax({
            type:"POST",
            url:"/iam/ajax/show_naver_map.php",
            data:{type:"distance", card_idx:card_idx},
            dataType: 'text',
            success:function(data){
                var addr = data.split('@');
    ...
    function calc_distance(){
        ...
        .then(data => {
            ...
            $.ajax({
                ...
                url:"/iam/ajax/show_naver_map.php",
                dataType:"json",
                data:{start_info:ret_val, type:'distance_calc', end_info:end_info},
                success:function(data){
    ...
    function show_map_position(card_idx){
        $.ajax({
            type:"POST",
            url:"/iam/ajax/show_naver_map.php",
            data:{type:"show_map", card_idx:card_idx},
            dataType: 'text',
            success:function(data){
    ...
    function save_set_reduce(type){
        ...
        $.ajax({
            type:"POST",
            url:"/iam/ajax/show_naver_map.php",
            data:{type:type, fixed:fixed, event_val:event_val, event_cnt:event_cnt, card_idx:card_idx},
            dataType: 'json',
            success:function(data){
                alert("설정 되었습니다.");
                //location.reload();
    ...
    function show_map_busitime(card_idx){
        $.ajax({
            type:"POST",
            url:"/iam/ajax/show_naver_map.php",
            data:{type:"busi_time", card_idx:card_idx},
            dataType: 'text',
            success:function(data){
                $("#timedata").html(data);
    ...
    function edit_busitime(){
        ...
        $.ajax({
            type:"POST",
            url:"/iam/ajax/show_naver_map.php",
            data:{type:"edit_busi_time", cont_idx:cont_idx},
            dataType: 'text',
            success:function(data){
                console.log(data);
    ...
    function start_auto_update(){
        ...
        if(confirm("포인트 차감 안내"+"\n"+"콘텐츠 오토데이트 기능을 사용하게 되면 선택한 IAM카드에서 발생하는 트래픽 비용으로 매월 1100원의 포인트가 차감됩니다. 포인트가 부족할 경우 오토데이트가 중지됩니다. 미리 포인트를 충분히 충전해두시기 바랍니다.")){
            ...
            $.ajax({
                ...
                dataType:"json",
                data:{address:address, my_id:my_id, contents_keyword:contents_keyword, sel_position:slt, short_url:sel_type, web_type:web_type, upload_time:upload_time, start_date:start_date, end_date:end_date},
                success:function (data) {
                    console.log(data);
                    if(data == 3){
    ...
    function edit_auto_update(){
        if(confirm("변경 하시겠습니까?")){
            ...
            $.ajax({
                ...
                url:"/iam/auto_update_contents.php",
                dataType:"json",
                data:{auto_id:auto_id, address:address, my_id:my_id, contents_keyword:contents_keyword, short_url:sel_type, web_type:web_type, upload_time:upload_time, start_date:start_date, end_date:end_date},
                success:function (data) {
                    console.log(data);
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                $.ajax({
                    ...
                    url:"/iam/auto_update_contents.php",
                    data:{
                        stop_auto:true, my_id:my_id, id:id
                    },
                    success:function(data){
                        if(data == 1){
                            alert("오토데이트가 중지상태로 되었습니다.");
    ...
    function del_send_list(val, val1){
        if(confirm("삭제하시겠습니까?")){
            $.ajax({
                ...
                url:"/iam/card_con_send.php",
                dataType:"json",
                data:{settle_type:"del", no:val, card_con:val1},
                success:function(data){
                    alert("삭제되었습니다.");
                    location.reload();
                }
    ...
    function search_send_list(val, val1, search_key){
        ...
        $.ajax({
            ...
            dataType:"html",
            data:{type:type, more:more, ID:'<?=$_SESSION[iam_member_id];?>', search_key:search_key},
            success: function(data){
                // console.log(data);
                if(type == "card")
    ...
    function receive_card(val){
        $.ajax({
            ...
            url:"/iam/card_con_send.php",
            dataType:"json",
            data:{settle_type:"recv_card", no:val, sender:'<?=$sender_id?>', settle_point:<?=$card_send_point?>},
            success:function(data){
                if(data == 3){
    ...
    function receive_contents(val1, val2){
        $.ajax({
            ...
            url:"/iam/card_con_send.php",
            dataType:"json",
            data:{settle_type:"recv_contents", no:val1},
            success:function(data){
                if(val2 != ""){
    ...
    function receive_notice(val1, val2){
        $.ajax({
            ...
            url:"/iam/card_con_send.php",
            dataType:"json",
            data:{settle_type:"recv_notice", no:val1},
            success:function(data){
                if(val2 != ""){
                    window.open(val2,'_blank');
                }
    ...
    function show_cnt(val){
        $.ajax({
            ...
            url:"/iam/card_con_send.php",
            dataType:"json",
            data:{settle_type:"show_cnt", no:val},
            success:function(data){
            }
        })
    }
    ...
    function delete_all(val){
        ...
        $.ajax({
            ...
            url:"/ajax/card_con_send_list.php",
            dataType:"json",
            data:{del_all:true, method:method, ID:'<?=$_SESSION[iam_member_id];?>'},
            success:function(data){
                // res = JSON.parse(data);
                console.log(data);
                if(val == "recv")
    ...
    function refuse(val){
        $.ajax({
            ...
            url:"/iam/card_con_send.php",
            dataType:"json",
            data:{settle_type:"refuse", no:val},
            success:function(data){
                alert("승인거부 되었습니다.");
                location.reload();
            }
    ...
    function remove_recv_notice(val){
        $.ajax({
            ...
            url:"/iam/card_con_send.php",
            dataType:"json",
            data:{settle_type:'remove_notice', no:val, ID:'<?=$_SESSION[iam_member_id];?>'},
            success: function(data){
                alert("삭제 되었습니다.");
                location.reload();
                // return;
    ...
    function search_auto(){
        ...
        $.ajax({
            ...
            dataType:"html",
            data:{search:true, start:start, end:end, ID:'<?=$_SESSION[iam_member_id];?>'},
            success: function(data){
    ...
    function post_contents(service_type) {
        ...
        if($("#btn_add_web_cont").hasClass("btn-active")){
            ...
            if ($("#contents_type").val() == 3 && $("#contents_price").val().trim() == "") {
                toastr.error("정가를 입력하세요.", "오류");
                $("#contents_price").focus();
                return;
                }
            if ($("#contents_type").val() == 3 && $("#contents_sell_price").val().trim() == "") {
                toastr.error("최저가를 입력하세요.", "오류");
                $("#contents_sell_price").focus();
                return;
            }
            var gwc_state = 0;
            gwc_state = $("#gwc_con_state").val();
            var formData = new FormData();
            var media_type = "I";
            ...
            if(($("#btn_add_shop_cont").hasClass("btn-active") || $("#contents_type").val() == 3) && mem_type == '1' && gwc_state != 0 || req_provide == 'Y'){
    ...
    function showCard(status) {
        $.ajax({
            type: "POST",
            url: "/iam/ajax/contents.proc.php",
            data: {"post_type":"show_iam_card", "mem_id":"mem_id","status":status},
            success: function(data) {
                alert(data);
                location.reload();
            }
    ...
    var popup_interval = setInterval(function(){
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#today_content-modalwindow").animate({bottom: '20px'},"slow");
                    $("#today_content-modalwindow").delay(2000);
                    $("#today_content-modalwindow").animate({bottom: '-80px'},"slow");
                }
            });
        }, <?=$alarm_time?> * 60 * 1000);
    function hideTodayContentModal(){
        clearInterval(popup_interval);
        $("#today_content-modalwindow").animate({bottom: '-80px'},"fast");
    ...
    function show_block_list(cont_idx){
        $.ajax({
            ...
            success: function (data) {
                $("#block_list_modal").modal({backdrop: 'static'});
    ...
    $(function(){
        ...
        $("#contents_percent").keyup(function(){
            ...
            var con_percent = $("#contents_percent").val();
        ...
        $("#contents_sell_price").keyup(function(){
            var con_price = $("#contents_price").val();
            var con_sell_price = $("#contents_sell_price").val();
