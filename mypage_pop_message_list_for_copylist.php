...
<Script>
function useIt(ori_sms_idx, event_name_eng, pcode, event_name_kor, event_idx, title, desc, mobile, type) {
    if(type == "creat"){
      opener.open_create_page(ori_sms_idx);
      window.close();
      return;
    }
    if(type == "change"){
      opener.open_change_page(ori_sms_idx);
      window.close();
      return;
    }
    ...
    opener.$('select[name=mobile]').val(mobile);
    try{
    ...
    try{
        opener.$('#mb_id_copy').val('<?=$_SESSION['one_member_id']?>');
    } catch(e) {
        
    }  
    try{
        opener.$('#mb_id').val('<?=$_SESSION['one_member_id']?>');
    } catch(e) {
