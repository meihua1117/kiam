...
if($REPLYCD == "0000"){//pay_test
    ...
    $password = $member_iam['mem_pass'];
    $cell = $member_iam['mem_phone'];
    ...
    if($row['member_type'] == "year-professional"){
        $sql_data = "select mem_phone, mem_point, mem_cash, mem_name from Gn_Member where mem_id='{$member_iam['mem_id']}'";
        $res_data = mysqli_query($self_con,$sql_data);
        $row_data = mysqli_fetch_array($res_data);
        
        $sql_res = "insert into Gn_Item_Pay_Result set buyer_id='{$member_iam['mem_id']}', buyer_tel='{$row_data['mem_phone']}', item_name='씨드포인트충전', item_price=1000000, pay_percent=90, current_point={$row_data['mem_point']}, current_cash={$row_data['mem_cash']}, pay_status='Y', VACT_InputName='{$row_data['mem_name']}', type='buy', seller_id='{$member_iam['mem_id']}', pay_method='결제씨드충전', pay_date=now(), point_val=1,billdate='{$APPLY_YMD}'";
    ...
    <?
        ...
        $sql = "insert into tjd_pay_result_month set pay_idx='{$row['idx']}',
                                                        ...
                                                        regdate = NOW(),
                                                        pay_yn='N',
                                                        msg='".iconv("euc-kr","utf-8",$REPLYMSG)."_mp_fix_iam_ajax"."',
                                                        amount='{$row['TotPrice']}',
                                                        buyer_id='{$member_iam['mem_id']}'";
    }
}else{
