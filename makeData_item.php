...
if (isset($_POST['point_val'])) {
    ...
    if (strpos($_POST['cont_idx'], ',') === false) { //굿마켓상품 한개만 주문하는 경우
        ...
        $_POST['month_cnt'] = 120;
        if (!$gwc || ($gwc && $item_price)) { //굿마켓결제이면서 카드(무통장)결제금액이 있거나 굿마켓 결제가 아닌 일반상품 결제인경우
            ...
            if (!isset($_POST['spgd'])) { //일반 서비스콘 구매가 아닌경우
                $sql = "insert into $pay_table set 
        ...
        if ($gwc) { //굿마켓결제
            $gwc_pay_status = "N";
            if ($use_point) { //이용포인트가 있으면 포인트 결제내역에 보여주기 위해 디비에 저장
                // $pay_idx_tjd = 0;
                if (!$item_price) {
                    $sql = "insert into $pay_table set 
                                idx='{$_POST['allat_order_no']}',
            ...
            if ($_POST['cont_idx'] == "54065") { //캐시상품결제인경우 캐시를 바로 추가
    ...
    } else { //굿마켓상품 여러개 한번에 결제하는경우(장바구니)
