						...
						?>
							...
							?>
								<tr>
									...
									<td><?= $row['pcode'] ?></td>
									<td>
										<a href="javascript:;;" onclick="useIt('<?= $row['event_title']; ?>','<?= $row['pcode'] ?>','<?= $row['event_idx'] ?>')">사용하기</a>
									</td>
								</tr>
							<?
								$sort_no--;
...
<Script>
	function useIt(event_title, pcode, event_idx) {
		opener.$('#pcode').val(pcode);
		opener.$('#event_title').val(event_title);
		try {
