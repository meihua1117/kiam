...
if($mode == "send_sms") {
        ...
        $sql="select count(*) cnt from Gn_Event_Check_Sms where mem_phone='$rphone' and date_format(regdate, '%Y-%m-%d' )=curdate() ";
        ...
        $sql="select * from Gn_Event_Check_Sms where REPLACE(mem_phone,'-','')=REPLACE('$rphone','-','') order by idx desc";
        $result = mysqli_query($self_con,$sql) or die(mysqli_error($self_con));
        $data = $row=mysqli_fetch_array($result);

        $sql="update Gn_Event_Check_Sms set status='N' where mem_phone='$rphone'";
        $result = mysqli_query($self_con,$sql) or die(mysqli_error($self_con));

        $sql="insert into Gn_Event_Check_Sms set mem_phone='$rphone', secret_key='$rand_num', regdate= NOW()";
        $result = mysqli_query($self_con,$sql) or die(mysqli_error($self_con));
       ...
       /******************** 인증정보 ********************/
        ...
        $sql="select * from Gn_Event_Check_Sms where mem_phone='$rphone' order by idx desc";
        $result = mysqli_query($self_con,$sql) or die(mysqli_error($self_con));
        $data = $row=mysqli_fetch_array($result);
