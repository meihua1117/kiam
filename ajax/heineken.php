...
if($user_id){
	...
	if($_POST['send_title'] && $_POST['send_txt'])
	{
        ...
        if(!count($sendnum)){
            echo json_encode(array('result'=>'fail', 'msg' => '발송가능한 휴대폰이 없습니다.'));
            exit;
        }
        ...
        $sql = "select phone_cnt from tjd_pay_result where buyer_id = '$user_id' and end_date > '$date_today' and end_status in ('Y','A') order by end_date desc limit 1";
        ...
        if(count($num_arr) > $thiMonleftCnt){
            echo json_encode(array('result'=>'fail', 'msg' => '이번 달 발송 가능 건수를 초과하였습니다.'));
            exit;
        }
        ...
        echo json_encode(array("result"=>"success", "msg" => count($success_cell_arr).'|'.count($no_num).'|'.$re_today_cnt.'|'.$ssh_total_cnt.'|'.count($deny_num)."|".count($etc_arr)."|".count($wrong_arr)."|".count($lose_arr)));
