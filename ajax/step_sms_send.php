...
$fp = fopen("step_sms_send".date("YmdHis").".log","w+");
      ...
      for ($i = 0; $i < count($ids_arr); $i++) {
        if (strpos($sms_idx, ",") !== false) {
          ...
          for ($j = 0; $j < count($sms_idx_arr); $j++) {
            insert_db($sms_idx_arr[$j], $ids_arr[$i],$fp);
          }
        } else {
          insert_db($sms_idx, $ids_arr[$i],$fp);
        }
      }
    } else {
      if (strpos($sms_idx, ",") !== false) {
        $sms_idx_arr = explode(",", $sms_idx);
        for ($j = 0; $j < count($sms_idx_arr); $j++) {
          insert_db($sms_idx_arr[$j], $ids,$fp);
        }
      } else {
        insert_db($sms_idx, $ids,$fp);
      }
    }
      ...
      for ($i = 0; $i < count($ids_arr); $i++) {
        if (strpos($req_idx, ",") !== false) {
          ...
          for ($j = 0; $j < count($req_idx_arr); $j++) {
            insert_db_reqlink($req_idx_arr[$j], $ids_arr[$i],$fp);
          }
        } else {
          insert_db_reqlink($req_idx, $ids_arr[$i],$fp);
        }
      }
    } else {
      if (strpos($req_idx, ",") !== false) {
        $req_idx_arr = explode(",", $req_idx);
        for ($j = 0; $j < count($req_idx_arr); $j++) {
          insert_db_reqlink($req_idx_arr[$j], $ids,$fp);
        }
      } else {
        insert_db_reqlink($req_idx, $ids,$fp);
      }
    }
...
function insert_db($idx, $mem_id,$fp)
{
  global $self_con;
  $date = time();
  $num = rand(10, 90);
...
function insert_db_reqlink($idx, $mem_id,$fp)
{
  global $self_con;
  $date = time();
  $num = rand(10, 90);
