...
function add_event_request($event_code, $event_idx, $pcode, $self_con)
{
...
if ($_POST['method'] == "create_format") {
    ...
    if ($row['request_yn']) {
        ...
        add_event_request($erq_row['event_name_eng'], $erq_row['event_idx'], $erq_row['pcode'], $self_con);
    }
        ...
        if ($mem_id != "") {
            ...
            if ($row['service_type'] >= 2) {
                foreach ($repo_arr as $repo) {
                    $sql = "select title,descript,sign_visible,pcode,request_yn from gn_report_form where id={$repo}";
