	...
	if ($row_p['mem_id']) {
		...
		if ($row_n_c['idx']) {
			...
			if (str_replace("-", "", $row['mem_phone']) == $userNum) {
				$sql_in = "insert into Gn_MMS_Number set sendnum = '$userNum', mem_id = '$userId', end_status='M',reg_date = now(),  end_date='{$row_pay['end_date']}' $addQuery"; //신규등록
				mysqli_query($self_con, $sql_in);
				$result = "0"; //0이 로그인 성공						
			} else {
				$sql_in = "insert into Gn_MMS_Number set sendnum = '$userNum', mem_id = '$userId', end_status='Y' ,reg_date = now(),  end_date='{$row_pay['end_date']}' $addQuery"; //신규등록
				mysqli_query($self_con, $sql_in);
				$result = "0"; //0이 로그인 성공				
			}
		}
