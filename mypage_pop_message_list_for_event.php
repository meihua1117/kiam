						...
						if ($intRowCount) {
							...
							?>
								<tr>
									...
									<td>
										<a href="javascript:;;" onclick="useIt('<?= $row['sms_idx']; ?>','<?= $row['reservation_title'] ?>')">사용하기</a>
									</td>
								</tr>
							<?
								$sort_no--;
...
<Script>
	function useIt(sms_idx, title) {
		opener.$('#step_idx' + <?= $addIndex ?>).val(sms_idx);

		try {
			opener.$('#reservation_title' + <?= $addIndex ?>).val(title);
		} catch (e) {
