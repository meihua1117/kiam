...
extract($_REQUEST);
	...
	*/
	...
	$sql="select idx,send_num,recv_num,up_date,reg_date,reservation,title,content,result,jpg,jpg1,jpg2 from $sql_table where $sql_serch order by $order_name $order_status limit $int,$intPageSize";
	                        ...
	                        <div class="sub_4_4_t4">
	                            <table class="list_table" width="100%" border="0" cellspacing="0" cellpadding="0">
	                                <tr>
	                                    ...
	                                    <td style="width:10%;">수신번호<?=$_REQUEST[status2]==''?"<br />(수신예약시간)":""?></td>
									...
									if($intRowCount)
									{
										...
										<tr>
											...
											<td style="font-size:12px;">
												<a href="javascript:void(0)" onclick="show_recv('show_recv_num','<?=$c?>','수신번호')"><?=str_substr($row[recv_num],0,14,'utf-8')?>
												<?=$row[reservation]?"<br>".$row[reservation]:""?></a> 
...
function selected_msg_del(){
	...
	if(confirm("삭제 하시겠습니까?")){
		$.ajax({
			...
			dataType:"json",
			data:{admin:0, delete_name:"mms_delete", id:idx_arr.toString()},
...
function all_msg_del(type){
	...
	if(confirm("모든 페이지 데이타를 모두 삭제합니다.  삭제하시겠어요?")){
		$.ajax({
			...
			url:"/admin/ajax/delete_func.php",
			dataType:"json",
			data:{admin:0, delete_name:"mms_delete", mem_id:'<?=$_SESSION[one_member_id]?>', type:type},
			success: function(data){
				console.log(data);
				if(data == 1){
					alert('삭제 되었습니다.');
