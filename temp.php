...
sendemail("","jungjungjae0909@naver.com","admin@kiam.kr","hello","hello");
...
while($row = mysqli_fetch_assoc($res)){
    ...
    if($row1['event_idx']){
        mysqli_query($self_con,"update Gn_Iam_Service set auto_join_event_idx = '{$row1['event_idx']}' where idx={$row['idx']}");
...
while($row = mysqli_fetch_assoc($res)){
	...
	$mobile = str_replace("-","",$row1['mem_phone']);
        mysqli_query($self_con,"update Gn_event_request set mobile = '{$mobile}' where request_idx = {$row['request_idx']}");
        echo "update ".$row['request_idx']."=>".$mobile."<br>";
    }else if(!$row1){
	mysqli_query($self_con,"delete from Gn_event_request where request_idx = {$row['request_idx']}");
...
while($card_row = mysqli_fetch_assoc($res)){
    ...
    $con_res = mysqli_query($self_con,$con_sql);
...
$res = mysqli_query($self_con,$sql);
while($mall_row = mysqli_fetch_assoc($res)){
