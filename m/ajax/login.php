...
if ($_POST['one_id'] && $_POST['one_pwd']) {
	...
	if ($row['mem_code'] && $row['is_leave'] == 'N') {
		...
		if ($_POST['refer2']) {
			...
			if($_POST['data'] != "")
				$url_refer .= "&data=".$_POST['data'];
?>
			<script language="javascript">
				window.parent.location.href = '<?= $url_refer ?>';
			</script>
		<? }
		...
		<? } else if ($row['site'] != "") {
			...
			if ($row['site'] == "kiam")
				$site = "";
		?>
			<iframe src="http://<?= $site ?>kiam.kr/ajax/login.support.php?one_id=<?=$_POST['one_id']; ?>&one_pwd=<?=$_POST['one_pwd']; ?>"></iframe>
			<script language="javascript">
				if (document.location.protocol == 'http:')
					var url = "http://" + "<?= $site ?>" + "kiam.kr/m/index.php?key=" + '<?= $sess_id ?>';
				else
					var url = "https://" + "<?= $site ?>" + "kiam.kr/m/index.php?key=" + '<?= $sess_id ?>';
