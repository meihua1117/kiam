        ...
        <div>
            ...
            <table class="list_table" width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:13%;">신청창키워드</td>
                <td style="width:20%;">메시지세트제목</td>
                <td style="width:20%;">메시지세트설명</td>
                <td style="width:10%;">발송폰번호</td>
                <td style="width:7%;">단계</td>
                <td style="width:6%">선택</td>
                
              </tr>
              <?

				$sql_serch=" m_id ='{$_SESSION['iam_member_id']}' ";
...
<Script>
function useIt(no, ori_sms_idx, event_name_eng, pcode, event_name_kor, event_idx, title, desc) {
  // console.log($(this).closest('tr').find(".select").val());
  // console.log($("select[name=step_num_"+no+"]").val());
    opener.$('#event_name_eng').val("");
    opener.$('#pcode').val(pcode);
    opener.$('#event_code').val(event_name_eng);
    opener.$('#sms_idx').val(ori_sms_idx);
    opener.$("#step_num").val($("select[name=step_num_"+no+"]").val());
    try{
    ...
    try{
        opener.$('#event_idx_').val(event_idx);
    } catch(e) {
        
    }    
    try{
        opener.$('#event_name_eng_event').val(pcode);
    } catch(e) {
    ...
    try{
        opener.$('#reservation_title').val(title);
    } catch(e) {
        
    } 
    try{
        opener.$('#reservation_desc').val(desc);
    } catch(e) {
        
    } 
    try{
        opener.$('#mb_id_copy').val('<?=$_SESSION['iam_member_id']?>');
    } catch(e) {
        
    }  
    try{
        opener.$('#mb_id').val('<?=$_SESSION['iam_member_id']?>');
    } catch(e) {
        
    }  
    try{
        opener.$('#ori_sms_idx').val(ori_sms_idx);
    } catch(e) {
        
    }   
    try{
        opener.$('#step_sms_title').val(title);
    } catch(e) {
