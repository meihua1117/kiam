 ...
 *
 * cannot be used as long as it could happen that the $goto file that is include_onced
 * at the end of this script is not updated to work without imported request variables
 *
 * @todo uncomment this if all possible include_onced files to rely on import request variables
...
if (isset($_REQUEST['insert_rows']) && is_numeric($_REQUEST['insert_rows']) && $_REQUEST['insert_rows'] != $cfg['InsertRows']) {
    ...
    $GLOBALS['js_include_once'][] = 'tbl_change.js';
    include_once './libraries/header.inc.php';
    include_once './tbl_change.php';
    exit;
}
...
if (isset($_REQUEST['after_insert'])
 && in_array($_REQUEST['after_insert'], array('new_insert', 'same_insert', 'edit_next'))) {
    ...
    $goto_include_once = 'tbl_change.php';
    ...
    if (! preg_match('@^[a-z_]+\.php$@', $GLOBALS['goto'])) {
        ...
        $goto_include_once = $GLOBALS['goto'];
    }
...
if (! $goto_include_once) {
    if (! strlen($GLOBALS['table'])) {
        $goto_include_once = 'db_sql.php';
    } else {
        $goto_include_once = 'tbl_sql.php';
    }
}
...
foreach ($loop_array as $rownumber => $where_clause) {
    ...
    foreach ($me_fields_name as $key => $field_name) {
        ...
        include_once './libraries/tbl_replace_fields.inc.php';
...
if ($is_insert && count($value_sets) > 0) {
    ...
    $active_page = $goto_include_once;
    if (! $GLOBALS['is_ajax_request'] == true) {
        include_once './libraries/header.inc.php';
    }
    include_once './' . PMA_securePath($goto_include_once);
    exit;
}
...
if ($GLOBALS['is_ajax_request'] == true) {
    ...
    if (isset($_REQUEST['do_transformations']) && $_REQUEST['do_transformations'] == true ) {
        ...
        foreach ($mime_map as $transformation) {
            $include_once_file = PMA_securePath($transformation['transformation']);
            ...
            foreach ($edited_values as $cell_index => $curr_cell_edited_values) {
                if (isset($curr_cell_edited_values[$column_name])) {
                    ...
                    if (file_exists('./libraries/transformations/' . $include_once_file)) {
                        $transformfunction_name = str_replace('.inc.php', '', $transformation['transformation']);

                        include_once './libraries/transformations/' . $include_once_file;
 ...
 * Load target page.
 */
require './' . PMA_securePath($goto_include_once);
exit;
?>
