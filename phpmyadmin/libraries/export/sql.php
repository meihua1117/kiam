...
if (isset($plugin_list)) {
    ...
    if (!$hide_sql) {
        ...
        $plugin_list['sql']['options'][] = array(
            ...
            'subgroup_header' => array(
                'type' => 'bool',
                'name' => 'include_once_comments',
                'text' => __('Display comments <i>(include_onces info such as export timestamp, PHP version, and server version)</i>')
            ));
        $plugin_list['sql']['options'][] = array(
            'type' => 'text',
            'name' => 'header_comment',
        ...
        $plugin_list['sql']['options'][] = array(
            ...
            'name' => 'insert_syntax',
            'values' => array(
                'complete' => __('include_once column names in every <code>INSERT</code> statement <br /> &nbsp; &nbsp; &nbsp; Example: <code>INSERT INTO tbl_name (col_A,col_B,col_C) VALUES (1,2,3)</code>'),
    ...
    {
        if (isset($GLOBALS['sql_include_once_comments']) && $GLOBALS['sql_include_once_comments']) {
     ...
     * @param bool    $show_dates     whether to include_once creation/update/check dates
    ...
    function PMA_getTableDef($db, $table, $crlf, $error_url, $show_dates = false, $add_semicolon = true, $view = false)
    {
        ...
        if ($result != false) {
            if (PMA_DBI_num_rows($result) > 0) {
                ...
                // but starting with MySQL 5.0.24, the clause is already include_onced
     ...
     * @param bool    $do_relation  whether to include_once relation comments
     * @param bool    $do_mime      whether to include_once mime comments
     ...
     * @param string  $crlf         the end of line sequence
     * @param string  $error_url    the url to go back in case of error
     * @param bool    $relation     whether to include_once relation comments
     * @param bool    $comments     whether to include_once the pmadb-style column comments
     *                                as comments in the structure; this is deprecated
     *                                but the parameter is left here because export.php
     *                                calls PMA_exportStructure() also for other export
     *                                types which use this parameter
     * @param bool    $mime         whether to include_once mime comments
     * @param bool    $dates        whether to include_once creation/update/check dates
