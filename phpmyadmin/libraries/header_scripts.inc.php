...
if ( false === $GLOBALS['cfg']['AllowThirdPartyFraming']) {
    echo PMA_include_onceJS('cross_framing_protection.js');
}
...
$GLOBALS['js_include_once'][] = 'jquery/jquery.qtip-1.0.0-rc3.js';
$params = array('lang' => $GLOBALS['lang']);
if (isset($GLOBALS['db'])) {
    $params['db'] = $GLOBALS['db'];
}
$GLOBALS['js_include_once'][] = 'messages.php' . PMA_generate_common_url($params);
// Append the theme id to this url to invalidate the cache on a theme change
$GLOBALS['js_include_once'][] = 'get_image.js.php?theme=' . urlencode($_SESSION['PMA_Theme']->getId());
...
$GLOBALS['js_include_once'] = array_unique($GLOBALS['js_include_once']);
foreach ($GLOBALS['js_include_once'] as $js_script_file) {
    echo PMA_include_onceJS($js_script_file);
}
