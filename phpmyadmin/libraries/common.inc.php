 ...
 * MUST be include_onced by every script
 *
...
if (isset($_POST['usesubform'])) {
     ...
     * include_once this page at the end of this script - we use $__redirect to
     * track this
     */
...
/**
 * include_once deprecated grab_globals only if required
 */
if (empty($__redirect) && !defined('PMA_NO_VARIABLES_IMPORT')) {
    include_once './libraries/grab_globals.lib.php';
}
...
/**
 * include_once session handling after the globals, to prevent overwriting
 */
...
if (! PMA_isValid($_REQUEST['token']) || $_SESSION[' PMA_token '] != $_REQUEST['token']) {
     ...
     */
    include_once './libraries/cleanup.lib.php';
    /**
     * Do actual cleanup
     */
...
/**
 * holds name of JavaScript files to be include_onced in HTML header
 * @global array $js_include_once
 */
$GLOBALS['js_include_once'] = array();
$GLOBALS['js_include_once'][] = 'jquery/jquery-1.6.2.js';
$GLOBALS['js_include_once'][] = 'jquery/jquery-ui-1.8.16.custom.js';
$GLOBALS['js_include_once'][] = 'update-location.js';

/**
 * holds an array of javascript code snippets to be include_onced in the HTML header
...
if (@file_exists($_SESSION['PMA_Theme']->getLayoutFile())) {
    include_once $_SESSION['PMA_Theme']->getLayoutFile();
    /**
...
if (!empty($__redirect) && in_array($__redirect, $goto_whitelist)) {
    /**
     * include_once subform target page
     */
    include_once $__redirect;
    exit();
}
?>
