...
function save_reg_cust(){
    ...
    if(confirm(msg)){
        $.ajax({
            ...
            data:{mode:mode,type:action_mode,idx:idx,mem_id:mem_id,name:name,phone1:phone1,phone2:phone2,email:email,birthday:birthday,work_type:work_type,company_name:company_name,job:job,company_addr:company_addr,home_addr:home_addr,link:link,memo:memo,list_idx:list_idx},
            success:function(){
                if(action_mode == "creat"){
                    alert('저장되었습니다.');
                }
...
function show_edit_input(val, type, list_id=0){
    if(type == "creat"){
        ...
        $.ajax({
            ...
            url:"/admin/ajax/manage_customers.php",
            dataType:'json',
            data:{mode:"edit_table",type:type, idx:val},
            success:function(data){
                if(type == "reg_cust_edit" || type == "member_reg_edit"){
...
input, select, textarea {
    ...
    <? include "header.php";?>
        ...
        $btn_hide = "hidden";
        $hide1 = "hidden";
        $color1 = "background-color:#a19d9d;";
        $hide2 = "hidden";
        $color2 = "background-color:#a19d9d;";
        $hide3 = "hidden";
        $color3 = "background-color:#a19d9d;";
                    ...
                    while($row = mysqli_fetch_array($res)) {
                        ...
                        if($row_mem_reg['mem_code'] != ''){
                            ...
                            <tr>
                                ...
                                <td><?=$com_add1?></td>
                                <td><?=$mem_add1?></td>
                                <td><a href="card_list.php?mem_id=<?php echo $row_mem_reg['mem_id']?>"><?=$card_url?></a></td>
        ...
        <?php if($_REQUEST['sel_tab'] == "get_cust"){?>
                ...
                <table id="example1" class="table table-bordered table-striped" style="width:100%;font-size:12px;">
                    ...
                    while($row = mysqli_fetch_array($res)) {
                        ...
                        <tr >
                            ...
                            <td><?=$addr?></td>
                            <td><?=$addr1?></td>
                            <td><a href="<?php echo $row['url']?>"><?=$short_url?></a></td>
                            <td><?=$row['regdate']?></td>
