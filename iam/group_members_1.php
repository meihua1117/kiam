        ...
        <main id="star" class="common-wrap" style="border:1px solid #ddd">
            <section id="middle">
                <div style="margin-top:10px;border-bottom:1px solid #ddd;display:flex;justify-content: space-between;padding-bottom: 5px;">
                    ...
                    <button type="button" class = "btn btn-primary" style="margin-right:10px" onclick = "delete_members()">탈퇴</button>
                </div>
                            ...
                            <div class="pagination" style="<?=$style_page?>">
                                <ul>
                                    ...
                                    if($page2 > 1) { //만약 page가 1보다 크다면
                                        ...
                                        echo "<li class='arrow'><a href='?search_type=$search_type&search_str2=$search_str2&search_range2=$search_range2&group=$group&page2=$pre2'><i class='fa fa-angle-left' aria-hidden='true'></i></a></li>";
                                    }
                                    for($i=$block_start2; $i<=$block_end2; $i++){
                                        ...
                                        if($page2 == $i) { //만약 page가 $i와 같다면
                                            ...
                                            echo "<li><a href='?search_type=$search_type&search_str2=$search_str2&search_range2=$search_range2&group=$group&page2=$i'>$i</a></li>"; //현재 페이지에 해당하는 번호에 굵은 빨간색을 적용한다
                                        }
                                    }

                                    if($block_num2 < $total_block2) { //만약 현재 블록이 블록 총개수보다 크거나 작다면
                                        $next2 = $page2 + 1; //next변수에 page + 1을 해준다.
                                        echo "<li class='arrow'><a href='?search_type=$search_type&search_str2=$search_str2&search_range2=$search_range2&group=$group&page2=$next2'><i class='fa fa-angle-right' aria-hidden='true'></i></a></li>"; //다음글자에 next변수를 링크한다. 현재 4페이지에 있다면 +1하여 5페이지로 이동하게 된다.
                                    }?>
...
<script>
    ...
    function delete_members(){
        ...
        /*$("input[type=checkbox]:checked").each(function() {
            if(mem_ids == "")
                mem_ids = $(this).data("mem_id");
            else
                mem_ids += "," + $(this).data("mem_id");
        });*/
        for (var i = 0; i < $("input[name=friends_chk]:checked").length; i++) {
            mem_ids = mem_ids + $("#friends_idx" + $("input[name=friends_chk]:checked").eq(i).val()).val() + ",";
        }
        mem_ids = mem_ids.substring(0, mem_ids.length - 1);
        if(mem_ids == "") {
            alert("탈퇴할 멤버들을 선택해주세요");
            return;
        }
        $.ajax({
            type: "POST",
