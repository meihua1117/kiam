...
<script>
    if ("<?= $platform ?>" != "mobile" && navigator.userAgent.toLocaleLowerCase().search("android") > -1) {
        location.href = "?platform=mobile&type=" + "<?= $type ?>";
    }
</script>
<?
if (!$_GET['type'])
...
<style type="text/css">
    ...
    .nav-tabs>.pay_tab.active>a,
    .nav-tabs>.pay_tab.active>a:focus,
    .nav-tabs>.pay_tab.active>a:hover {
        color: #000 !important;
    ...
    .exp .table-wrap .view_table tbody tr td:first-child {
        text-align: left
    }
    ...
    .exp .table-wrap .view_table thead tr th:first-child {
        text-align: left
    }
</style>
...
<script language=Javascript>
    ...
    function result_submit_no(result_cd, result_msg, enc_data) {
        ...
        if (result_cd == '0000') { //pay_test
            ...
            if (pay_form.member_type.value == "standard") {
                pay_form.allat_enc_data.value = enc_data;
                pay_form.action = "/allat/mp/allat_fix_iam.php";
                pay_form.method = "post";
                pay_form.target = "_self";
                pay_form.submit();
            } else {
                $.ajax({
                    type: "POST",
                    url: "/allat/mp/allat_fix_iam_ajax.php",
                    data: $('#pay_form').serialize(),
                    dataType: 'html',
                    success: function(data) {
                        $("#ajax-loading").delay(10).hide(1);
    ...
    function show_modal() {
        // var pay_modal_state_iam = getCookie('pay_modal_state_iam');
    ...
    function pay(frm) {
        // var pay_modal_state_iam = getCookie('pay_modal_state_iam');
        // if(pay_modal_state_iam == ''){
        //    setCookie("pay_modal_state_iam","done",1,"iam");
                ...
                if ($('#allat_recp_addr').val() == "") {
                    $.ajax({
                        ...
                        error: function() {
                            ...
                            location.href = "mypage.php?link=/iam/pay";
                            return;
                        }
                    });
                } else {
                ...
                if ($('#allat_recp_addr').val() == "") {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('#pay_form').attr("action", "/allat/mp/allat_fix_iam.php");
    ...
    function form_save() {
        if ($('#mem_id').val() == "") {
            ...
            alert('이미 등록된 도메인입니다. 다른 도메인을 입력해주세요.');
            $('#sub_domain').focus();
        } else {
            $.ajax({
                type: "POST",
...
<body style="padding-top: 0px">
    <div class="big_main pay-wrap" style="height: auto;min-height: 100%;background:white;margin-bottom: 50px;">
        <div id="wrap" class="common-wrap" style="max-width: 1024px;padding:0px">
            <!--/div>
        <div style = "max-width: 1024px;padding-top: 0px"-->
            <div style="margin-top: 86px;text-align:center">
                <!--img src="/images/sub_02_visual_03.jpg" style = "width : 100%"/-->
                <label style="color:#82c836;font-size:18px;margin-top:20px;margin-bottom:10px">결제안내</label>
            </div>
            ...
            <section id="pay_menu" style="background: white">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li class="nav-item exp-tab pay_tab" style="width :33%" onclick="activeTab(1)">
                        <a class="nav-link" id="exp-tab" data-toggle="tab" href="#" role="tab" aria-controls="exp" aria-selected="true">서비스 구성</a>
                    </li>
            ...
            <div class="pay" style="border: 1px solid #ddd;">
                <form name="pay_form" id="pay_form" method="post"> <!--승인요청 및 결과수신페이지 지정 //-->
                    ...
                    <!--아이엠결제타입-->
                    <!--input type="hidden" name="iam_pay_type" id="iam_pay_type" value=''-->
                    <!--아이엠카드갯수-->
                    <input type="hidden" name="iam_card_cnt" id="iam_card_cnt" value=''>
                    <!--아이엠공유갯수-->
                    ...
                    <div style="display:flex;">
                        ...
                        <div class="form-check-inline">
                            <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="pay_type" value="year">약정결제
                            </label>
                        </div>
                        <div class="form-check-inline">
                            <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="pay_type" value="all" checked>정기결제
                            </label>
                        </div>
                        <div class="form-check-inline">
                            <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="pay_type" value="dber">디버결제
                            </label>
                        </div>
                    </div>
                                ...
                                <div class="col-12">
                                    <table class="view_table" cellspacing="0" cellpadding="0" data-num="3" style="font-size:14px">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td class="colored fw-bold active" colspan="3" style="position:relative">
                    ...
                    <? } else { ?>
                        <div class="a8"><a href="javascript:show_modal();"><img src="/images/sub_02_btn_23.jpg" /></a></div>
                    <? } ?>
                </form>
            </div>
            <div class="cancel" style="background : white;display: none">
                <div style="font-weight: 700;font-size: 16px;padding: 20px;">
                    ...
                    <span style="color:red;">
                        ...
                        . 해지신청을 하지 않고 탈퇴를 해도 결제는 유지됩니다. 탈퇴를 하시기 전 반드시 해지신청을 하시길 바랍니다.<br>
                    </span><br><br>

                    <h4><b>[약정결제 해지안내]</b></h4>
                    ∙ 약정결제는 3년간 3.3만원 결제로 30%의 수익을 배분 받는 애즈회원을 말합니다.<br>
                    ∙ 약정결제 해지를 하려면 마이페이지>> 결제정보에서 해지신청을 해주세요. 이때 보여지는 위약금액수를 확인하세요. <br>
                    <span style="color:red;">
                        ∙ 해지 신청을 하신 후에 본사에 확인하신 위약금을 납부해야 해지가 됩니다.<br>
                        ∙ 혹시 위약금 처리가 부담되면 다른 약정고객을 구하여 양도하실수 있습니다.
            ...
            <div class="modal-dialog" style="width: 100%;max-width:768px;">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer" style="padding:0px;display:flex">
                        <button type="button" class="btn btn-default btn-left" style="width:50%;" onclick="pay(document.pay_form)">잘 확인했습니다</button>
            ...
            <div class="modal-dialog" style="width: 100%;max-width:768px;">
                ...
                <div class="modal-content">
                    <div class="modal-header" style="border:none;background-color: rgb(130,199,54);border-top-right-radius: 5px;border-top-left-radius: 5px;">
                        ...
                        <div class="login_bold" style="margin-bottom: 0px;color: #ffffff;font-size: 22px;text-align: center">마이플랫폼 구축 정보</div>
                    </div>
                    <div class="modal-body">
                        <div class="container" style="margin-top: 20px;text-align: center;width:100%">
                            <form method="post" id="dForm" name="dForm" action="/admin/ajax/service_Iam_save.php" enctype="multipart/form-data">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            <th>분양자아이디</th>
                                            <td> <input type="text" style="width:100%;" name="mem_id" id="mem_id" value="<?= $Gn_mem_row['mem_id'] ?>"> </td>
                                        </tr>
                                        <tr>
                                            <th>분양자이름</th>
                                            <td> <input type="text" style="width:100%;" name="mem_name" id="mem_name" value="<?= $Gn_mem_row['mem_name'] ?>"> </td>
                                        </tr>
                                        <tr>
                                            <th>업체대표이름</th>
                                            <td> <input type="text" style="width:100%;" name="owner_name" id="owner_name" value="<?= $Gn_mem_row['mem_name'] ?>"> </td>
                                        </tr>
                                        <tr>
                                            <th>관리자이름</th>
                                            <td> <input type="text" style="width:100%;" name="manager_name" id="manager_name" value="<?= $Gn_mem_row['mem_name'] ?>"> </td>
                                        </tr>
                                        <tr>
                                            <th>업체이름</th>
