...
if(isset($_POST['my_id'])){
    ...
    if($set_position != 1){
        ...
        $res_name = mysql_query($sql_mem_name);
    ...
    if($set_position == 2){
        ...
        $res_idx = mysql_query($sql_card_idx);
        ...
        $row_dup = mysql_fetch_array($res_dup);
    }
    else{
        $card_idx = 0;
    }
    ...
    if(isset($_POST['auto_id'])){//오토데이트 편집
        ...
        $res_state = mysql_query($sql_check_state);
        ...
        $res_ori_history = mysql_query($sql_ori_history);
        $row_ori_history = mysql_fetch_array($res_ori_history);
        ...
        if(($set_position == 2) && ($row_dup['card_idx'] != NULL)){
            ...
            mysql_query($sql_release);

            $sql_rest_point = "select * from Gn_Member where mem_id='{$mem_id}'";
            $res_rest_point = mysql_query($sql_rest_point);
            $row_rest_point = mysql_fetch_array($res_rest_point);
