...
<head>
    ...
    <meta property="og:title" content="그룹으로 초대하기">
    <!--제목-->
    <meta property="og:description" content="그룹초대">
    <!--내용-->
    <title>그룹초대</title>
...
<body>
    <div id="wrap" class="common-wrap">
        <main id="star" class="common-wrap" style="border:1px solid #ddd">
            <section id="middle">
                <div style="margin-top:10px;border-bottom:1px solid #ddd">
                    <h3 style="margin-left:10px;">공개프로필 정보리스트</h3>
                </div>
                        ...
                        <div class="search-box clearfix J2" id="friends_search" style="padding: 10px 15px;background-color: #ffffff;">
                            <div class="row" style='margin-left: 8px;margin-bottom: 10px;margin-top: 7px;'>
                                ...
                                <div class="right" style="display: flex;">
                                    <button class="btn btn-primary" style="margin-right: 10px" onclick = "invite()">초대</button>
                            ...
                            <div class="row" style = "line-height : 30px">
                                ...
                                <div style="float:right;padding-top:5px;padding-left:20px;">
                                    <?if($page_type == "all"){?>
                                        <a href="javascript:friends_submit('page');"><img src ="img/main/icon_all_green.png" style="width: 20px"></a>
                                    <?}else{?>
                                        <a href="javascript:friends_submit('all');"><img src ="img/main/icon_all_gray.png" style="width: 20px"></a>
                                    <?}?>
                        ...
                        <div class="inner">
                            <div class="contact-list" style="<?=$style_list?>">
                                <ul>
                                    <?
                                    if($search_str2 !== "")
                                        $friends_sql_msg = "and (mem_name like '%$search_str2%' or mem_phone like '%$search_str2%' or mem_id like '%$search_str2%')";
                                    else
                                        $friends_sql_msg = "";
                                    ...
                                    if((int)$search_type == 1)
                                        $sql = "select mem_code, mem_id, mem_phone, profile, zy, mem_name from Gn_Member where mem_id not in (select mem_id from gn_group_member where group_id='$group') and mem_id not in (select invite_id from gn_group_invite where group_id='$group') $friends_sql_msg";
                                    else
                                        $sql = "select mem_code, mem_id, mem_phone, profile, zy, mem_name from Gn_Member where mem_id not in (select mem_id from gn_group_member where group_id='$group') and mem_id not in (select invite_id from gn_group_invite where group_id='$group') and site_iam='$site_iam' $friends_sql_msg";
                                    if($page_type == "page"){
                            ...
                            <div class="pagination" style="<?=$style_page?>">
                                <ul>
                                    ...
                                    if($page2 > 1) { //만약 page가 1보다 크다면
                                        ...
                                        echo "<li class='arrow'><a href='?group=$group&search_type=$search_type&search_str2=$search_str2&search_range2=$search_range2&page2=$pre2'><i class='fa fa-angle-left' aria-hidden='true'></i></a></li>";
                                    }
                                    for($i=$block_start2; $i<=$block_end2; $i++){
                                        ...
                                        if($page2 == $i) { //만약 page가 $i와 같다면
                                            ...
                                            echo "<li><a href='?group=$group&search_type=$search_type&search_str2=$search_str2&search_range2=$search_range2&page2=$i'>$i</a></li>"; //현재 페이지에 해당하는 번호에 굵은 빨간색을 적용한다
                                        }
                                    }

                                    if($block_num2 < $total_block2) { //만약 현재 블록이 블록 총개수보다 크거나 작다면
                                        $next2 = $page2 + 1; //next변수에 page + 1을 해준다.
                                        echo "<li class='arrow'><a href='?group=$group&search_type=$search_type&search_str2=$search_str2&search_range2=$search_range2&page2=$next2'><i class='fa fa-angle-right' aria-hidden='true'></i></a></li>"; //다음글자에 next변수를 링크한다. 현재 4페이지에 있다면 +1하여 5페이지로 이동하게 된다.
                                    }?>
                                </ul>
                            </div>
                            <?}?>
                            <div class = "button-group" style="display: flex;justify-content: center;">
                                ...
                                <button class="btn btn-primary" style="margin-left: 10px" onclick = "invite()">초대</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function friends_range(v1) {
        if (Number($("#search_range2").val()) == 1 && Number(v1) == 1) {
            location.href =
                "?group=<?=$group?>&page_type=<?=$page_type?>&search_type=<?=$search_type?>&search_range2=2&search_str2=<?=$search_str2?>"
        } else if (Number($("#search_range2").val()) == 2 && Number(v1) == 1) {
            location.href =
                "?group=<?=$group?>&page_type=<?=$page_type?>&search_type=<?=$search_type?>&search_range2=1&search_str2=<?=$search_str2?>"
        } else if (Number($("#search_range2").val()) == 3 && Number(v1) == 3) {
            location.href =
                "?group=<?=$group?>&page_type=<?=$page_type?>&search_type=<?=$search_type?>&search_range2=4&search_str2=<?=$search_str2?>"
        } else if (Number($("#search_range2").val()) == 4 && Number(v1) == 3) {
            location.href =
                "?group=<?=$group?>&page_type=<?=$page_type?>&search_type=<?=$search_type?>&search_range2=3&search_str2=<?=$search_str2?>"
        } else {
            location.href =
                "?group=<?=$group?>&page_type=<?=$page_type?>&search_type=<?=$search_type?>&search_range2=" + v1 +"&search_str2=<?=$search_str2?>"
        }
    }
    function friends_submit() {
        location.href = "?group=<?=$group?>&page_type=<?=$page_type?>&search_type=" + $("#search_type").val() + "&search_str2=" + $("#search_str2").val();
    }
    function friends_submit(page_type) {
        location.href = "?group=<?=$group?>&page_type="+page_type+"&search_type=" + $("#search_type").val() + "&search_str2=" + $("#search_str2").val();
    }
    function invite(){
        var mem_array = "";
        ...
        for (i = 0; i < $("input[name=friends_chk]:checked").length; i++) {
            mem_array = mem_array + $("#friends_idx" + $("input[name=friends_chk]:checked").eq(i).val()).val() + ",";
        }
        mem_array = mem_array.substring(0, mem_array.length - 1);
        if(mem_array.length == 0){
            alert("초대할 아이디를 선택해주세요.");
            return;
        }
        $.ajax({
            type: "POST",
