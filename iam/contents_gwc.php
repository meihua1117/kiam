...
$meta_sql="update Gn_Iam_Contents_Gwc set contents_temp=contents_temp+1 where idx = '$contents_idx'";
mysqli_query($self_con, $meta_sql);

$meta_sql="select * from Gn_Iam_Contents_Gwc where idx = '$contents_idx'";
...
<body>
			...
			<section id="bottom">
				...
				<div class="content-item" id="contents<?=$meta_row['idx']?>">
					...
					<div class="media-wrap">
						<div class="media-inner" <?if($meta_row['contents_type'] == 1 && count($content_images) == 0) echo "style = 'min-height :30px'"; else echo "style = 'max-height: 650px'"?>>
						<div class="user-item">
							...
							<div class="dropdown" style="position: absolute; right: 10px; top: 8px;">
								<!-- <button class="btn-link dropdown-toggle westory_dropdown" onclick="close_win()" type="button" style="font-size: 20px;">
									X
								</button> -->
									...
									<div class="desc-inner">
										<div class="outer <?=$row_card['sale_cnt_set']?>" style="text-align:left;">
											...
											<?//}else{
												...
												<!-- <div class="percent" style="<?=$style_decor?>"><?=(int)$discount?>%</div> -->
											...
											<? }else{?>
												<div class="price" style="width:230px;display: inline-flex;">
													...
													<span class="downer" style="color:black;font-size: 18px;margin-right: 3px;"><?=number_format($meta_row['contents_price'])?>원</span>
													<span class="downer" style="font-size: 12px;color:darkgrey;"><span style="vertical-align: text-bottom;<?=$price_style?>"><?=number_format($meta_row['contents_price'])?></span>원</span>
												</div>
											...
											<!-- <div class="price_help" style="position: absolute;right: 100px;top: 37%;<?=$buy_btn?><?=$buy_btn_con?>">
												<a style="border: 1px solid black;padding:0px 5px;cursor: pointer;border-radius: 10px;font-size: 13px;" href="javascript:price_help_show()">i</a>
											</div> -->
					...
					<div class="info-wrap" style="border: none;position: fixed;bottom: 0px;width: 97%;max-width: 765px;">
						...
						<div class="second-box">
							...
							<div class="in-box" style="position: absolute;right: 10px;">
								<a class="gwc_order_btn1" href="javascript:show_order_option('<?=$meta_row['idx']?>', 'cart', '<?=$gwc_mem?>')" style="margin-bottom: 5px;font-size: 18px;border: 1px solid #99cc00;margin-right: 10px;padding: 3px 10px;border-radius: 10px;">장바구니</a>
								<a class="gwc_order_btn2" href="javascript:show_order_option('<?=$meta_row['idx']?>', 'pay', '<?=$gwc_mem?>')" style="margin-bottom: 5px;font-size: 18px;padding: 3px 10px;border-radius: 10px;border: 1px solid;color: white;background-color: #99cc00;">바로구매</a>
							</div>
							<?}?>
						</div>
					</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="center_text">
						...
						<div class="sns_item" onclick="contents_sendSMS()"><div class="sns_icon_div"><img class="sns_icon" src = "img/sns_icon2.png"></div><div class="sns_item_text" >문자<br>보내기</div></div>
						<div class="sns_item" onclick="contents_shareKakaoTalk()"><div class="sns_icon_div"><img class="sns_icon" src = "img/sns_icon3.png"></div><div class="sns_item_text" >카톡<br>공유</div></div>
					</div>
					<div class="center_text">
						...
						<div class="sns_item" onclick="contents_shareFaceBook()"><div class="sns_icon_div"><img class="sns_icon" src = "img/sns_icon5.png"></div><div class="sns_item_text" >페북<br>공유</div></div>
						<div class="sns_item" onclick="contents_copyContacts()"><div class="sns_icon_div"><img class="sns_icon" src = "img/sns_icon4.png"></div><div class="sns_item_text" >주소<br>복사</div></div>
					</div>
				</div>
			</div>
		</div>
	...
	<!--div id="post_popup" class="modal fade" tabindex="-1" role="dialog">
		...
		<div class="modal-dialog modal-sm" role="document" style="margin-top: 10%;width : 50%;max-width:400px;margin-left:auto;margin-right:auto;">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div>
						<div class="login_text">
							...
							&#8226;인신공격, 비판적이거나 회의적인 글, 다른 개인이나 단체에 대한적대적인 글을 게시해서는 안됩니다.<br>
							&#8226;외설적이거나 저속한 말, 비속어 또는 비속어를 암시하는 말 및줄임말, 욕설, 징종차별적인 비방을 해서는 안됩니다.<br>
							&#8226;외부 웹사이트로 이동하는 링크는 게시하지 않습니다.<br>
							&#8226;다른 출처에서 복사해 붙여넣기를 한다면, 해당 출처를 명시해야 합니다.<br>
							&#8226;저작권 보호를 받는 자료는 게시할 수 없습니다.<br>
							&#8226;상업적 홍보나 광고는 게시할 수 없습니다.<br>
							2. 댓글 작성 지침<br>
							&#8226;아이엠 로그인을 해주세요.<br>
							&#8226;댓글은 300자 이내로 등록하실 수 있습니다.<br>
							&#8226;최근 24시간 내 댓글 20개, 답글 수 40개로 제한됩니다.<br>
							&#8226;콘텐츠 당 작성 가능한 댓글은 최대 3개입니다.<br>
							&#8226;댓글/답글은 60초 내에 한 개만 등록하실 수 있습니다.<br>
						</div>
					</div>
				</div>
...
<script>
	...
	function close_win(){
		if(mobile == "Y"){
			...
			if(order == "Y"){
				window.parent.document.getElementById("container").style.display = "flow-root";
			}
			else{
		...
		try{
			Kakao.Link.sendDefault({
				...
				content: {
					...
					link: {
						mobileWebUrl: '<?=$domainData['sub_domain'];?>/iam/contents_gwc.php?gwc=Y&contents_idx=' + idx, // 모바일 카카오톡에서 사용하는 웹 링크 URL
						webUrl: '<?=$domainData['sub_domain'];?>/iam/contents_gwc.php?gwc=Y&contents_idx=' + idx // PC버전 카카오톡에서 사용하는 웹 링크 URL
					}
				},
				buttons: [{
	...
	function shareFaceBook_contents(idx, name) {
		...
		var linkUrl = encodeURIComponent('<?=$domainData['sub_domain'];?>/iam/contents_gwc.php?gwc=Y&contents_idx=' + idx);
	...
	function contents_copy(idx) {
		...
		// 지정된 요소의 값을 할당 한다.
		aux.setAttribute("value", "<?=$domainData['sub_domain'];?>/iam/contents_gwc.php?gwc=Y&contents_idx=" + idx);
		// bdy에 추가한다.
	...
	function show_post(content_idx){
		...
		//}
		var post_wrap_ht = $('.post-wrap').height();
		var media_ht = win_ht * 1 - ft_ht * 1 - post_wrap_ht * 1 - 30;
		$('.media-inner').attr('style', 'max-height:'+media_ht+'px');
	}
