...
if($mode == "check_model_name"){
    ...
    $sql_this_month_pay = "select SUM(TotPrice) as money from tjd_pay_result_gwc where cash_prod_pay=0 and buyer_id='{$_SESSION['iam_member_id']}' and end_status='Y' and date>'{$date_this_month}'";//당월 구매금액
    ...
    $sql_pre_month_pay = "select SUM(TotPrice) as money from tjd_pay_result_gwc where cash_prod_pay=0 and buyer_id='{$_SESSION['iam_member_id']}' and end_status='Y' and date>'{$date_pre_month}' and date<'{$date_this_month}'";//전달 구매금액
