...
if($_POST[join])
{
	...
	if($e_id != "kiam"){
		...
		$id_res = mysql_query($sql_id);
		$row_id = mysql_fetch_array($id_res);
		$id = $row_id['mem_id'];
	}
	else{
		$id = "obmms02";
	...
	$res_ncard = mysql_query($sql_namecard);
	$row_ncard = mysql_fetch_array($res_ncard);
