...
if ($mall_type == "main_mall") {
    $sql8 = "select mall.* from Gn_Iam_Mall mall where mall.display_status = 1 ";
} else if ($mall_type == "sub_mall") {
    $site_info = explode(".", $HTTP_HOST);
    $site_name = $site_info[0];
    $sql8 = "select mall.* from Gn_Iam_Mall mall where mall.display_status = 1 and mem.site_iam = '$site_name'";
} else if ($mall_type == "my_mall") {
    $sql8 = "select mall.* from Gn_Iam_Mall mall where mall.mem_id = '{$_SESSION['iam_member_id']}'";
} else if ($mall_type == "best_mall") {
    $sql8 = "select mall.* from Gn_Iam_Mall mall where mall.sample_display = 'Y'";
} else if ($mall_type == "my_mall_like") {
    $sql8 = "select mall.* from Gn_Iam_Mall mall where (mall.mall_like ='{$_SESSION['iam_member_id']}' or mall.mall_like like '{$_SESSION['iam_member_id']},%' or mall.mall_like like '%,{$_SESSION['iam_member_id']}' or mall.mall_like like '%,{$_SESSION['iam_member_id']},%')";
}
...
while ($contents_row = mysqli_fetch_array($result8)) {
    ...
    if ($contents_row['mall_type'] == 1) {
        ...
        */
        ...
        $preview_link = "/iam/contents.php?contents_idx=" . $contents_row['card_idx'];
    ...
    if ($contents_row['mem_id'] == $_SESSION['iam_member_id']) {
        ...
        $body .= '<img src="/iam/img/menu/icon_utils_edit.png" style="width:20px;height:20px;">';
        $body .= '</a>';
        $body .= '<a class="content-utils" style="margin-right: 5px;" href="javascript:delete_mall(\'' . $contents_row['idx'] . '\');"><img src="/iam/img/menu/icon_utils_del.png" ></a>';
        $body .= '<a class="content-utils" href="javascript:get_mall_link(\'' . $contents_row['idx'] . '\');" >';
        $body .= '<img src="/iam/img/menu/icon_utils_share.png"  style="width:20px;height:20px;">';
        $body .= '</a>';
        $body .= '</div>';
        ...
        $body .= '<div style="display:flex;justify-content: flex-end;flex-direction: row-reverse;float: right;margin-top:5px;margin-right:5px">';
        $body .= '<a class="content-utils" href="javascript:get_mall_link(\'' . $contents_row['idx'] . '\');" style="cursor:pointer;z-index:10;margin-right: 5px;margin-top:14px;">';
        $body .= '<img src="/iam/img/menu/icon_utils_share.png"  style="width:13px;height:16px;margin-left:3px;margin-top:-1px;">';
        $body .= '</a>';
        if (strstr($contents_row['mall_like'], $_SESSION['iam_member_id'])) {
            $body .= '<a class="content-utils" style="cursor:pointer;z-index:10;" ';
            $body .= 'onclick="unlike_mall(\'' . $contents_row['idx'] . '\')">';
