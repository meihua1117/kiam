...
if ($search_key)
    ...
    $sql8 = "select count(idx) from Gn_Iam_Con_Card WHERE card_idx = '$card_idx'";
...
if ($cont_count > 0 && !$gwc_table) {
    $sql_gwc = "select idx from Gn_Iam_Contents_Gwc ct WHERE card_idx = $card_idx and public_display='Y' and " . $search_sql;
    $sql_gwc .= $order_sql;
    $sql_gwc .= " limit 0,300";
    $res_gwc = mysqli_query($self_con, $sql_gwc);
    while ($row_gwc = mysqli_fetch_array($res_gwc)) {
        array_push($cont_id_array, $row_gwc[0]);
    }
}
...
if ($search_key)
    ...
    $sql8 = "select ct.* from Gn_Iam_Contents ct INNER JOIN Gn_Iam_Con_Card cc on cc.cont_idx = ct.idx WHERE cc.card_idx = $card_idx and $search_sql $order_sql";

if ($w_offset < count($cont_id_array)) {
    $temp_cont_arr = array_slice($cont_id_array, $w_offset, $contents_count_per_page);
    ...
    $contents_count_per_page = $w_offset + $contents_count_per_page - count($cont_id_array);
