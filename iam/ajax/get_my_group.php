...
} else if ($_GET['sort'] == 12) { //조회수
    $order_sql = " ORDER BY contents_temp desc";
} else if ($_GET['sort'] == 13) { //좋아요
...
if ($cont_order == 1)
    $order_sql .= ',ct.contents_order desc';
else if ($cont_order == 2)
    $order_sql .= ',ct.req_data desc';
else if ($cont_order == 3)
    $order_sql .= ',ct.wm_date desc';
else if ($cont_order == 4)
        ...
        if ($content_images != null) {
            if ($cur_card['share_send_card'] != 0 || $_SESSION['iam_member_id'] != $contents_row['mem_id']) {
                ...
                $body .=        "<a onclick=\"show_contents_utils(" . $contents_row['idx'] . ");\" class=\"content-utils\"" .
                    ...
                    "<div>";
                if ($_SESSION['iam_member_id'] && ($_SESSION['iam_member_id'] == $contents_row['mem_id'] || $_SESSION['iam_member_id'] == $_GET['group_manager'])) {
                    if ($_SESSION['iam_member_id'] == $group_manager) {
                        $img = $contents_row['group_fix'] > 0 ? '/iam/img/main/icon-pin.png' : '/iam/img/main/icon-graypin.png';
                        $body .=                    "<a class=\"content-utils\" onclick=\"contents_fix('" . $contents_row['idx'] . "');\">" .
                    ...
                    if ($_SESSION['iam_member_id'] == $contents_row['mem_id']) {
                        ...
                        if ($is_pay_version) {
                            $body .=                        "<a class=\"content-utils\" onclick=\"show_con_send('" . $contents_row['idx'] . "')\">" .
                                "<img src=\"/iam/img/menu/icon_utils_send.png\"  style=\"\">" .
                                "</a>";
                        }
                    }
                }
