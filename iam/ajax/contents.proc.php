...
if ($post_type == "creat") {
    ...
    $res_share_new = mysql_query($sql_share_new);
    $cnt_share_new = mysql_num_rows($res_share_new);
    if ($contents_add_multi_free == 1) {
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            foreach ($card_array as $card_link) {
                ...
                $sql2 = "update Gn_Iam_Name_Card set up_data=now() where idx={$card}";
                mysql_query($sql2) or die(mysql_error());
            }
        }

        if ($cnt_share_new) {
            while ($row_share_new = mysql_fetch_array($res_share_new)) {
                ...
                for ($i = 0; $i < $total; $i++) {
                    ...
                    $sql2 = "update Gn_Iam_Name_Card set up_data=now() where idx={$row_share_new['idx']}";
                    mysql_query($sql2) or die(mysql_error());
                }
            }
        }
        ...
        if ($contents_type == 3) {
            ...
            $spec_res = mysql_query($sql2);
            $spec_row = mysql_fetch_assoc($spec_res);
            ...
            if ($cnt_share_new) {
                while ($row_share_new = mysql_fetch_array($res_share_new)) {
                    ...
                    $sql2 = "insert into " . $contents_table_name . " (mem_id,contents_type,contents_img,contents_title,contents_url,contents_url_title,
                                                        ...
                                                        contents_footer_display, contents_temp,contents_share_text,contents_share_count,req_data,up_data,
                                                        card_short_url,westory_card_url,public_display,card_idx,source_iframe,contents_order,landing_mode,
                                                        gwc_con_state,share_send_cont,reduce_val,media_type,open_type,";
                    if ($group_id != "NULL") {
                        $sql2 .= "group_id,";
                    }
                    if ($product_seperate != "") {
                    ...
                    $sql2 .= "group_display) values (\"{$row_share_new['mem_id']}\",\"$contents_type\",\"$contents_img\",\"$contents_title\",\"$contents_url\",\"$contents_url_title\", 
                                                        ...
                                                        \"$contents_display\",\"$contents_westory_display\",\"$contents_user_display\",\"$contents_type_display\", 
                                                        \"$contents_footer_display\", \"$contents_temp\",\"$contents_share_text\",$contents_share_count,now(),now(),
                                                        \"{$row_share_new['card_short_url']}\",\"{$row_share_new['card_short_url']}\",\"$public_display\",\"{$row_share_new['idx']}\",
                                                        \"$source_iframe\",$contents_order,\"$landing_mode\",'$gwc_con_state','$contents_idx','$reduce_val','$media_type',{$open_type},";
                    if ($group_id != "NULL") {
                        $sql2 .= "'$group_id',";
                    }
                    if ($product_seperate != "") {
...
// Update
    ...
    $sql_price = "select contents_price, contents_sell_price from " . $contents_table_name . " where idx = '$contents_idx'";
    ...
    if ($contents_img_url) {
        ...
        $old_res = mysql_query($old_sql);
        $old_row = mysql_fetch_array($old_res);
        ...
        if ($cnt_chk) {
            while ($row_chk = mysql_fetch_array($res_chk)) {
                ...
                $res_share_con = mysql_query($sql_share_con);
                $row_share_con = mysql_fetch_array($res_share_con);
                $sql2 = "update " . $contents_table_name . " set contents_type = '$contents_type',
                                    media_type = '$media_type',
                ...
                $sql2 = "update Gn_Iam_Name_Card set up_data=now() where idx={$row_share_con['card_idx']}";
                mysql_query($sql2) or die(mysql_error());
            }
        }
    ...
    if (/*$_POST['art_type'] > 0 && */$contents_table_name == "Gn_Iam_Contents" && $contents_type == 3) {
        ...
        $mall_row = mysql_fetch_assoc($mall_res);
        if ($mall_row['idx'] != "") {
            ...
            if ($_POST['art_type'] == 0) {
                ...
                $sql2 = "select card_idx from $contents_table_name where idx = {$contents_idx}";
                $res2 = mysql_query($sql2);
                $row2 = mysql_fetch_assoc($res2);
                $sql2 = "update Gn_Iam_Name_Card set up_data=now() where idx={$row2['card_idx']}";
                mysql_query($sql2) or die(mysql_error());
...
// Remove Shared link
    ...
    $sql2 = "update Gn_Iam_Contents set contents_share_text ='$result',contents_share_count = $share_count,up_data=now() where idx = '$contents_idx'";
    mysql_query($sql2) or die(mysql_error());
...
// Delete
    ...
    $cnt_share = mysql_num_rows($res_share);
    if ($cnt_share) {
        while ($row_share = mysql_fetch_array($res_share)) {
...
// 이미지 삭제
    ...
    $sql = "select card_idx,contents_img from Gn_Iam_Contents where idx = '$contents_idx' and mem_id = '$mem_id'";
    $result = mysql_query($sql);
    $row = mysql_fetch_array($result);
    ...
    $sql = "update Gn_Iam_Contents set contents_img = '$contents_img',up_data=now() where idx = '$contents_idx' and mem_id = '$mem_id'";
    mysql_query($sql) or die(mysql_error());

    $sql = "update Gn_Iam_Name_Card set up_data=now() where idx ={$row['card_idx']}";
    mysql_query($sql) or die(mysql_error());
    echo "이미지가 삭제 되었습니다.";
    exit;
}
...
// 특정컨텐츠 감추기
    ...
    $result = mysql_query($sql);
    $iam_info = mysql_fetch_array($result);
...
// 특정유저의 컨텐츠 감추기
    ...
    if ($_POST['gwc_state'] == "gwc_prod") {
        ...
        $res_chk_limit = mysql_query($sql_chk_limit);
        $row_chk_limit = mysql_fetch_array($res_chk_limit);
        ...
        mysql_query($sql_store_con) or die(mysql_error());
        ...
        $res_ori_idx = mysql_query($sql_ori_idx);
        $row_ori_idx = mysql_fetch_array($res_ori_idx);
