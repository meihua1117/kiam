...
if($mode == "del") {
  ...
  for($i=0; $i<count($_POST['friends_idx']); $i++) {
    ...
    // exit;
    $sql="delete from Gn_Iam_Friends where idx = $friends_idx and (mem_id = '{$_SESSION['iam_member_id']}' or friend_id = '{$_SESSION['iam_member_id']}')";
...
//프렌즈 카드정보
  ...
  //프렌즈 카드 저장
  $sql4="insert into Gn_Iam_Friends (mem_id, friend_id, friends_card_idx, friends_name, friends_company, friends_position, friends_phone, friends_img, friends_url, req_data) 
  values ('{$_SESSION['iam_member_id']}','$receiver_id', '$card_idx', '$friends_name', '$friends_company', '$friends_position', '$friends_phone', '$friends_img', '$friends_url', now())";
  ...
  $sql8="select count(idx) from Gn_Iam_Friends where mem_id = '$receiver_id' and friends_card_idx = '$my_card_idx'";
  $result8=mysqli_query($self_con, $sql8);
  $comment_row8=mysqli_fetch_array($result8);

  if($comment_row8[0] == 0) {
    $sql7="insert into Gn_Iam_Friends (mem_id, friend_id, friends_card_idx, friends_name, friends_company, friends_position, friends_phone, friends_img, friends_url, req_data) 
    values ('$receiver_id', '$receiver_id','$my_card_idx', '$my_friends_name', '$my_friends_company', '$my_friends_position', '$my_friends_phone', '$my_friends_img', '$my_friends_url', now())";
  ...
  for($i=0; $i<count($_POST['friends_idx']); $i++) {
    ...
    if($receiver_id == $_SESSION['iam_member_id']){
      continue;
    }
    //프렌즈 카드 저장
    $sql4="insert into Gn_Iam_Friends (mem_id, friend_id, friends_card_idx, friends_name, friends_company, friends_position, friends_phone, friends_img, friends_url, req_data) 
