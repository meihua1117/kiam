...
<?
	...
	//$rsJoinDate = mysql_fetch_row($res_result);
    ...
    //오늘 예약 건 확인
	$reserv_cnt_today=0;
	$sql_result2 = "select recv_num from Gn_MMS where reservation like '$date_today%' and up_date is null and mem_id = '$_SESSION[one_member_id]' ";
	$res_result2 = mysql_query($sql_result2);
	while($row_result2 = mysql_fetch_array($res_result2))
		$reserv_cnt_today += count(explode(",",$row_result2[recv_num]));
...
//-오늘발송 건 수
	$rec_cnt_today=0;
	$sql_result2 = "select recv_num from Gn_MMS where reg_date like '$date_today%' and mem_id = '$_SESSION[one_member_id]' ";
	$res_result2 = mysql_query($sql_result2);
	while($row_result2 = mysql_fetch_array($res_result2))
		$rec_cnt_today += count(explode(",",$row_result2[recv_num]));
	mysql_free_result($res_result2);
//-이번발송 $uni_id
	$rec_cnt_current=0;
...
//마지막 발송 건수
	$sql_result32 = "select recv_num from Gn_MMS where mem_id = '$_SESSION[one_member_id]' and uni_id like '$uni_id%'";
...
//$memberInfo=mysql_fetch_array($result);
...
?>
	...
	function show_mail_box(){
		...
		$("#modal_mail").css({"top":coord.top , "left": coord.left + 100});
		$("#modal_mail").show();
					...
					<div>
						<div class="sub_4_2_t1">
							...
							<div class="div_240">
									...
									<div class="b4">
										...
										<div class="div_2px">
											<div style="float:left; display:none;" id="delay">
												...
												<input type="text" name="delay2" placeholder="발송간격" value="<?=$_REQUEST[delay] ? $_REQUEST[delay] : 15?>" onblur="send_delay(sub_4_form,this,2)" />초</div>
								...
								<div class="a2">
									<div class="b1 popup_holder" style="cursor:pointer";>부가서비스
                                        ...
                                        <div class="popupbox" style="display:none; height: 85px;width: 214px; left: 87px; top: -10px; line-height:1.5;">
                                            ...
                                            <a style="color: blue;" href="https://tinyurl.com/2p9b5jaf" target="_blank">[자세히 보기]</a>
                                        </div>
									</div>
								...
								<div class="a2">
									...
									<div class="div_2px">
										...
										<input type="file" name="upimage2" onChange="onUploadImgChange(this);sub_4_form.action='up_image_.php?i=0&k=2&target=upimage_str2';sub_4_form.target='excel_iframe';sub_4_form.submit();" accept=".jpg,.jpeg,.png,.gif"/></div>
								...
								<div>
									<?if($fujia_pay == "Y"){?>
										...
										<a href="javascript:void(0)" class="popbutton1">
											<img src="images/btn_send_help3.gif" />
										</a>
									<?
									}
									else
								...
								<div class="a4">
									...
									<div class="div_2px">
										...
										<div style="float:left;margin-left:30px;">
											<label><input type="checkbox" name="fs_msg" id="fs_msg" onclick="deny_msg_click(this,4)" />발신전용안내</label>
							...
							<div class="div_340">
								<div class="d_font popup_holder" style="width:150px;float:left;margin-top:0px; cursor:pointer;">
                                    ...
                                    <div class="popupbox" style="display:none; left:30px; top:30px; height: 84px;width: 214px; line-height:1.5;">
                                        ...
                                        <a style="color: blue;" href="https://tinyurl.com/y496wbp8" target="_blank">[자세히 보기]</a>
                                    </div>
                                ...
                                <div style="float:right;" class="popup_holder">
                                    ...
                                    <div class="popupbox" style="display:none; left:30px; top:30px; height: 85px;width: 218px; line-height:1.5;">여기를 클릭하면 자신의 폰 연락처 디비를 가져와서 자동으로 주소록을 만들어줍니다.<br>
                                        <a style="color: blue;" href="https://tinyurl.com/3fdbvu7k" target="_blank">[자세히 보기]</a>
                                    </div>
                                </div>
								...
								<div style="margin-bottom:5px;margin-top:5px;">
									...
									<table class="list_table" width="100%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											...
											<td style="width:40%;"><a href="javascript:order_sort(sub_4_form,'grp',sub_4_form.order_status.value)">그룹명<? if($_REQUEST[order_name]=="grp"){echo $_REQUEST[order_status]=="desc"?'▼':'▲';}else{ echo '▼'; }?></a></td>
											<td style="width:20%;"><a href="javascript:order_sort(sub_4_form,'reg_date',sub_4_form.order_status.value)">날짜<? if($_REQUEST[order_name]=="reg_date"){echo $_REQUEST[order_status]=="desc"?'▼':'▲';}else{ echo '▼'; }?></a></td>
											<td style="width:15%;"><a href="javascript:order_sort(sub_4_form,'count',sub_4_form.order_status.value)">인원</a><? if($_REQUEST[order_name]=="count"){echo $_REQUEST[order_status]=="desc"?'▼':'▲';}else{ echo '▼'; }?></td>
										...
										if($intRowCount){
											...
											while($row=mysql_fetch_array($result)){
												...
												<tr>
													<td>
                                                        <label>
                                                            <input type="checkbox" value="<?=$row[idx]?>" name="chk" id="chk" onchange="group_choice('<?=$g?>')" />
                                                            <?=$sort_no?>
                                                        </label>
                                                    </td>
						...
						<div class="sub_4_2_t3">
							...
							<div class="div_280" style="margin-top:0px">
								...
								<div style="margin:0px 0 20px 0;height:1200px;overflow-y:auto;">
                                    ...
                                    $sql = "select * from Gn_MMS where mem_id='".$_SESSION[one_member_id]."' and DATE(reg_date)='".$date."' and  content like '%app_check_process%' and result=0 group by send_num order by reg_date desc";
									$resul=mysql_query($sql);
									...
									<table class="list_table" width="100%" border="0" cellspacing="0" cellpadding="0" style="position:relative;top:10px;">
                                        <thead>
                                            <tr>
                                                <td style="width:35%;text-align:left;"><label><input type="checkbox" onclick="check_all(this,'go_num');send_sj_fun()" />휴대폰 번호</label></td>
										...
										if($intRowCount)
										{
											...
											while($row=mysql_fetch_array($resul))
											{
												...
												$is_send=true;
												$sql_result2_g = "select recv_num from Gn_MMS where send_num='$row[sendnum]' and ((reg_date like '$today_reg%' and reservation is null) or reservation like '$today_reg%')";
												$res_result2_g = mysql_query($sql_result2_g) or die(mysql_error());
												$today_cnt_1=0;
												while($row_result2_g = mysql_fetch_array($res_result2_g))
													$today_cnt_1+=count(explode(",",$row_result2_g[recv_num]));
		...
		<table class="table table-bordered" style="width: 97%;">
			<tbody>
				...
				<tr>
					...
					<td style="width: 30%;"><div style="float: left;"><label><input type="checkbox" style="padding:5px" id="chk_save" name="chk_save" />메시지 저장</label></div></td>
                </tr>
			</tbody>
		</table>
