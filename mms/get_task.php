...
$mms_row = mysqli_fetch_array($query);
mysqli_free_result($query);

$msg = str_replace("{|name|}", "{|REP|}", $mms_row['content']);
$msg = str_replace("{|email|}", "{|REP1|}", $msg);
//$msg = json_encode($msg);
$title = str_replace("{|name|}", "{|REP|}", $mms_row['title']);
...
if ($query) {
	$upt_sql = "update Gn_MMS set result = '0', up_date = now() where idx = {$mms_row['idx']}";
...
if (strstr($msg, "{|REP|}"))
	...
	$rep = "";
$mms_row['jpg'] = str_replace("null", "", $mms_row['jpg']);
$data = array(
	"txt" => $msg,
	"reqid" => $mms_row['uni_id'],
	"delay" => $mms_row['delay'],
	"delay2" => $mms_row['delay2'],
	"close" => 24,
	"idx" => $mms_row['idx'],
	"type" => $mms_row['type'],
	"title" => $title,
...
if (strpos($mms_row['recv_num'], ",")) {
	$receive = explode(",", $mms_row['recv_num']);
	$replacement1 = explode(",", $mms_row['replacement1']);
	$replacement2 = explode(",", $mms_row['replacement2']);
	$url_ex = explode(",", $url);
	$string = array();
	...
	$array = array(
		"bnc" => $url,
		"num" => $mms_row['recv_num'],
		"rep" => $mms_row['replacement1'],
		"rep1" => $mms_row['replacement2']
	);
	array_push($string, $array);
	$data['pnum'] = $string;
	$data = json_encode($data);
...
if ($mms_row['idx']) {
	$sql_j = "insert Gn_MMS_Json set mms_idx = '" . $mms_row['idx'] . "', data = '" . $data . "', reg_date = now()";
