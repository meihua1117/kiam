...
if (isset($_POST['method'])) {
    ...
    $method = $_POST['method'];
    if ($method == "get") {
        $sql = "select callback_times from Gn_Member use index(mem_id) where mem_id='{$userid}'";
        $res = mysql_query($sql);
        $row = mysql_fetch_assoc($res);
        echo json_encode(array("type" => substr($row['callback_times'],1)*1));
    } else if ($method == "set") {
        $type = "s".$_POST['type'];
        $sql = "update Gn_Member set callback_times='{$type}' where mem_id='{$userid}'";
        $res = mysql_query($sql);
        if($res)
    ...
    if ($callback_idx == 0) {
        ...
        $res_call = mysql_query( $sql);
        if (mysql_num_rows($res_call)) {
            $callback_state = 0;
            ...
            if ($phone_callback == $callback_idx && $phone_sel_state) {
                $callback_state = 1;
            }
            if ($mun_callback == $callback_idx && $mun_sel_state) {
                $callback_state = 2;
            }
    ...
    while ($rows = mysql_fetch_array($res1)) {
        ...
        $callback_state = 0;
        if ($phone_callback == $rows['idx'] && $phone_sel_state) {
            $callback_state = 1;
        }
        if ($mun_callback == $rows['idx'] && $mun_sel_state) {
            $callback_state = 2;
        }
    ...
    while ($rows_mem = mysql_fetch_array($res2)) {
        ...
        $callback_state = 0;
        if ($phone_callback == $rows_mem['idx'] && $phone_sel_state) {
            $callback_state = 1;
        }
        if ($mun_callback == $rows_mem['idx'] && $mun_sel_state) {
            $callback_state = 2;
        }
