            ...
            ?>
              ...
              ?>
                <tr>
                  ...
                  <td>
                    <a href="javascript:;;" onclick="useIt('<?= $row['event_name_eng']; ?>','<?= $row['pcode'] ?>','<?= $row['event_name_kor'] ?>','<?= $row['event_idx'] ?>')">사용하기</a>
                  </td>
                </tr>
              <?
                $sort_no--;
...
<Script>
  function useIt(event_name_eng, pcode, event_name_kor, event_idx) {
    try {
    ...
    try {
      opener.$('#event-idx').val(event_idx);
    } catch (e) {
