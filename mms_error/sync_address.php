...
$address1 = substr($address, 0, 255);
$sql = "insert into call_app_log (api_name, mem_id, send_num, recv_num, sms, regdate) values ('sync_address', '$id', '$id', '$mem_id', '$address1', now())";
mysqli_query($self_con,$sql);
fwrite($GLOBALS['fp'], "call_app_log 2::" .$sql.  "\n");
...
mysqli_query($self_con,$query);
$query = "delete from gn_iam_contacts where mem_id='$mem_id' or contact_id='$mem_id'";
mysqli_query($self_con,$query);
fwrite($GLOBALS['fp'], "delete_iam_contact::" .$query.  "::addr_cnt::".count($addr)."\n");
$acnt = 0;
...
foreach($iam as $key=>$value) {
	if($mem_id != "") {
        ...
        $query = "select mem_id from Gn_Member where mem_phone='$value[num]' and is_leave='N'";
        $res = mysqli_query($self_con,$query);
        $row = mysqli_fetch_array($res);
        if($row[mem_id]){
    	    $contact_id = $row[mem_id];
    	    $query = "select count(no) from gn_iam_contacts where mem_id='$mem_id' and contact_id='$contact_id'";
    	    $res = mysqli_query($self_con,$query);
    	    $row = mysqli_fetch_array($res);
    	    if($row[0] == 0){
    		$query = "insert into gn_iam_contacts set mem_id='$mem_id', contact_id='$contact_id',reg_date=NOW()";
    		$res = mysqli_query($self_con,$query);
    	    }
        }
        $acnt++;
