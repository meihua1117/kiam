...
{	
	if(confirm('로그아웃하시겠습니까?'))
	  {	
	   $.ajax({
			 type:"POST",
			 url:"/ajax/ajax_session_2016.php",
			 data:{logout_go:"로그아웃가자"},
			 success:function(data){ajax_state=1;$("#ajax_div").html(data)}
			})
	  }
}
//로그인체크
...
var iss = {
		...
		{
                if (document.getElementsByName('pwd')[i].value.length < 6 || document.getElementsByName('pwd')[i].value.length > 15){iss.level4();return}	
                var lv = -1;
                if (document.getElementsByName('pwd')[i].value.match(/[a-z]/ig)){lv++;}
                if (document.getElementsByName('pwd')[i].value.match(/[0-9]/ig)){lv++;}
                if (document.getElementsByName('pwd')[i].value.match(/([!@#$%^&*()_+])/ig)){lv++;}
                switch(lv)
                {                                
                        case 0:
                                iss.level0();
...
function g_dt_fun(status,g_id,idx,cnt)
{
	if(status!="del")
	{
		...
		{
		    document.getElementsByName('num_name')[cnt].value="이름"; // cooper modify 2016-04-15
...
{
    var str="";    
    if(!frm.upimage.value.match(/\.(gif|jpg|png)$/i)) { alert("이미지파일을 첨부 해주세요!"); return; }
    f.submit();
}
...
function show_msg(i)
{
	...
	document.getElementsByName('lms_content')[0].value=$($(".msg_content")[i]).html();
	document.getElementsByName('lms_content')[0].focus();
}
//포토저장 이동
...
function pay_total(status)
{
	...
	switch(status)
	{
		...
		case "add_service":
			...
			document.getElementsByName('add_money')[1].value=20000;
			else
			document.getElementsByName('add_money')[1].value=0;
			$($(".add_money_span")[1]).	html(number_format(document.getElementsByName('add_money')[1].value));		
		break;	
	}
	var price=parseInt(document.getElementsByName('add_money')[0].value)+parseInt(document.getElementsByName('add_money')[1].value);
	...
	for(var i=0; i<type_arr.length; i++)
	{
		if(type_arr[i].checked)
		{
			...
			var goodname="온리원문자 - 직접결제 - "+type_arr[i].value+"개월기간 ";
			$($(".auto_pay_c")[0]).hide();
			}
			else
			{
			document.getElementsByName('mid')[0].value='obmms20152';
			var goodname="온리원문자 - 자동결제 - "+type_arr[i].value+"개월기간 ";
			$($(".auto_pay_c")[0]).show();			
			}
			document.getElementsByName('goodname')[0].value=goodname;
			document.getElementsByName('month_cnt')[0].value=type_arr[i].value;
		}
	}
}
//결제고고
