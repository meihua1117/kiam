...
function id_check(frm, frm_str) {
  ...
  if (!pattern.test(frm.id.value)) {
    ...
    frm.id.focus();
    return;
  } else document.getElementById("id_html").innerHTML = "";
  $.ajax({
    type: "POST",
    url: "ajax/ajax.php",
    data: {
...
function nick_check(frm, frm_str) {
  ...
  var pattern = /[가-힣\x200-9]/gi;
  //var pattern = /[^\u4e00-\u9fa5]/i;
  if (!pattern.test(frm.nick.value)) {
    document.getElementById("nick_html").innerHTML =
      "한글 혹은 숫자만 입력가능합니다.";
...
function join_check(frm, modify) {
  ...
  if (frm.id) id_str = frm.id.value;
  var msg = modify ? "수정하시겠습니까?" : "등록하시겠습니까?";
  var email_str =
    frm.email_1.value + "@" + frm.email_2.value + frm.email_3.value;
  if (!modify)
    phone_str =
      frm.mobile_1.value + "-" + frm.mobile_2.value + "-" + frm.mobile_3.value;
  var birth_str =
    frm.birth_1.value + "-" + frm.birth_2.value + "-" + frm.birth_3.value;
...
function pwd_check(i) {
  var iss = {
    ...
    text: [
      "약 <img src='/images/check.gif' />",
      "중 <img src='/images/check.gif' />",
      "강 <img src='/images/check.gif' />",
      "검사중..",
      "비밀번호는 6~15 자내로 입력해야 합니다.",
    ],
    width: ["50", "100", "150", "10"],
    reset: function () {
  ...
  if (regexp.test(document.getElementsByName("pwd")[i].value)) {
    if (
      document.getElementsByName("pwd")[i].value.length < 6 ||
      document.getElementsByName("pwd")[i].value.length > 15
    ) {
      iss.level4();
      return;
    }
    var lv = -1;
    if (document.getElementsByName("pwd")[i].value.match(/[a-z]/gi)) {
      lv++;
    }
    if (document.getElementsByName("pwd")[i].value.match(/[0-9]/gi)) {
      lv++;
    }
    if (
      document.getElementsByName("pwd")[i].value.match(/([!@#$%^&*()_+])/gi)
    ) {
      lv++;
    }
    switch (lv) {
...
function pwd_cfm_check(i) {
  ...
  if (
    document.getElementsByName("pwd_cfm")[i].value !=
    document.getElementsByName("pwd")[i].value
  ) {
    ...
    if (document.getElementsByName("pwd_status")[i].value)
...
function num_del() {
  ...
  for (var i = 0; i < name_arr.length; i++) {
    if (name_arr[i].checked) num_a.push(name_arr[i].value);
  }
  if (!num_a.length) {
  ...
  if (g) {
    if (name_arr[g].checked) {
      ...
      for (var a = 0; a < group_arr.length; a++) {
        if (name_arr[g].value == group_arr[a]) group_arr.splice(a, 1);
      }
    }
  } else {
    for (var i = 0; i < name_arr.length; i++) {
      if (name_arr[i].checked) {
        ...
        for (var a = 0; a < group_arr.length; a++) {
          if (name_arr[i].value == group_arr[a]) group_arr.splice(a, 1);
        }
      }
    }
  }
  //document.getElementsByName('group_num')[0].value=group_arr.concat(group_arr).join(",");
...
function box_ed(g) {
  ...
  if (window.parent.document.getElementsByName("num")[0].value)
    group_arr = window.parent.document
      .getElementsByName("num")[0]
      .value.split(",");
  if (g) {
    if (name_arr[g].checked) {
      if (jQuery.inArray(name_arr[g].value, group_arr) == -1)
  ...
  window.parent.document.getElementsByName("num")[0].value =
    group_arr.join(",");
...
function show_detail(src, i) {
  ...
  $($(".group_title_open")[0]).html(
    document.getElementsByName("group_title")[i].value
  );
...
//디테일 수정 보이기
function g_dt_show_cencle(c1, c2, c3, i, c4) {
  if (c1) {
    $($("." + c1 + i)[0]).hide();
    $($("." + c1 + i)[1]).show();
  }
...
function search_id_pw(frm) {
  ...
  var phone_s = "";
  var email_s = "";
  if (frm.mem_name) mem_name_s = frm.mem_name.value;
  if (frm.mem_id) mem_id_s = frm.mem_id.value;
  ...
  if (confirm("찾으시겠습니까??")) {
    ...
    $.ajax({
      ...
      data: {
        ...
        search_id_pw_type: frm.serch_type.value,
      },
      success: function (data) {
        $($(".loading_div")[0]).hide();
        $("#ajax_div").html(data);
...
function numchk(status) {
  ...
  for (var i = 0; i < go_num_arr.length; i++) {
    if (go_num_arr[i].checked) send_num_a.push("`" + go_num_arr[i].value + "`");
  }
...
function send_sj_fun() {
  ...
  if (document.getElementsByName("ssh_check")[1].checked) numchk("5");
}
//all 그룹삭제
...
function onUploadImgChange(sender) {
  ...
  if (sender.files && sender.files[0]) {
    ...
    sender.select();
    document
      .getElementById("preview_fake")
      .filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src =
      document.selection.createRange().text;
    document
      .getElementById("preview_size_fake")
      .filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src =
      document.selection.createRange().text;
    autoSizePreview(
      document.getElementById("preview_fake"),
      document.getElementById("preview_size_fake").offsetWidth,
      document.getElementById("preview_size_fake").offsetHeight
    );
...
function textCounter(theField, theCharCounter, maxChars, b) {
  ...
  for (var i = 0; i < intLength; i++) {
    ...
    if (strCharCounter < maxChars + 1)
      ...
      eval(
        "alert('통신사 문자발송 글자수제한에 따라 한글" +
          maxChars / 2 +
          ", 영문" +
          maxChars +
          "자 제한입니다. 초과된 문자는 잘립니다.')"
      );
...
//엑셀다운로드
function excel_down(action, e, box_name) {
  if (box_name) {
    var box_arr = [];
    var box_a = [];
    var box_s = "";
    ...
    for (var i = 0; i < box_arr.length; i++) {
      if (box_arr[i].checked) box_a.push("`" + box_arr[i].value + "`");
    }
    box_s = box_a.join(",");
    if (box_s == "") {
      alert("적어도 하나는 선택해야 합니다.");
  ...
  if (e) excel_down_form.grp_id.value = e;
...
function select_app_check(dan_num) {
  var num_a = [];
  if (dan_num) num_a.push(dan_num);
  else {
    var num_arr = document.getElementsByName("seq[]");
    for (var i = 0; i < num_arr.length; i++) {
      if (num_arr[i].checked) num_a.push(num_arr[i].value);
    }
    if (!num_a.length) {
      alert("적어도 하나는 선택해야 합니다.");
      return;
...
function move_parent(status) {
  if (status == "photo") {
    if (document.getElementsByName("photo_content")[0].value) {
      window.opener.document.getElementsByName("upimage_str")[0].value =
        document.getElementsByName("photo_content")[0].value;
      window.opener.document.getElementsByName("title")[0].value =
        document.getElementsByName("title")[0].value;
      $($(".img_view", window.opener.document)[0]).html(
        "<img src='" +
          document.getElementsByName("photo_content")[0].value +
          "' />"
      );
    ...
    if (document.getElementsByName("lms_content")[0].value) {
      window.opener.document.getElementsByName("txt")[0].value =
        document.getElementsByName("lms_content")[0].value;
      window.opener.document.getElementsByName("title")[0].value =
        document.getElementsByName("title")[0].value;
      window.close();
    } else document.getElementsByName("lms_content")[0].focus();
  }
...
function move_parent2() {
  if (onebook_form.one_content.value) {
    ...
    window.opener.type_check();
    window.close();
  } else onebook_form.one_content.focus();
}
...
function show_msg(i) {
  ...
  document.getElementsByName("lms_content")[0].value = $($(".msg_content")[i])
    .html()
    .replace("&gt;", ">")
    .replace("&lt;", "<");
...
function show_photo(i) {
  ...
  document.getElementsByName("photo_content")[0].value =
    document.getElementsByName("photo_content")[i].value;
  $($(".img_view")[0]).html(
    "<img src='" + document.getElementsByName("photo_content")[i].value + "' />"
  );

  if (document.getElementsByName("photo_content1")[i].value) {
    document.getElementsByName("photo_content1")[0].value =
      document.getElementsByName("photo_content1")[i].value;
    $($(".img_view")[1]).html(
      "<img src='" +
        document.getElementsByName("photo_content1")[i].value +
        "' />"
  ...
  if (document.getElementsByName("photo_content2")[i].value) {
    document.getElementsByName("photo_content2")[0].value =
      document.getElementsByName("photo_content2")[i].value;
    $($(".img_view")[2]).html(
      "<img src='" +
        document.getElementsByName("photo_content2")[i].value +
        "' />"
...
function show_content(i) {
  document.getElementsByName("one_content")[0].value = $($(".a3_1")[i])
    .html()
    .replace(/<br \/>|<br>/g, "\n");
...
function book_select(i, name) {
  var url =
    "www.onepagebook.net/opb/app/sub/sub_gallery_view.php?it_id=" +
    document.getElementsByName("it_id")[i].value;
  onebook_url = "\n\n■ 원북 보러가기:" + url;
  var str1 = "[" + name + "님이 추천하는 원북]<br>";
  var str2 =
    "■ 제목:" + document.getElementsByName("it_name")[i].value + "<br>"; //제목
...
function show_recv(name, c, t, status) {
  if (!document.getElementsByName(name)[c].value) return;
  open_div(open_recv_div, 100, 1, status);
  if (name == "show_jpg")
    $($(".open_recv")[0]).html(
      "<img src='" + document.getElementsByName(name)[c].value + "' />"
    );
    ...
    $($(".open_recv")[0]).html(
      document.getElementsByName(name)[c].value.replace(/\n/g, "<br/>")
    );
...
function ml_view(name, c, t) {
  ...
  if (document.getElementsByName("fs_msg")[0].checked)
    content =
      document.getElementsByName(name)[c].value +
      document.getElementsByName("onebook_url")[c].value +
      "\n\n" +
...
function pay_total(status) {
  ...
  switch (status) {
    ...
    case "add_term":
      ...
      //$('#money3').val(($('#add_term').val() * 1) * 2000);
      //$('#sum3').html($('#money3').val());
      break;
    case "add_phone":
      if (!document.getElementsByName(status)[0].value) add_phone_cnt = 0;
      else {
        if (isNaN(document.getElementsByName(status)[0].value)) {
          document.getElementsByName(status)[0].value = "";
          add_phone_cnt = 0;
        } else add_phone_cnt = document.getElementsByName(status)[0].value;
      }
      document.getElementsByName("add_money")[0].value =
        parseInt(add_phone_cnt) * 2000;
      $($(".add_money_span")[0]).html(
        number_format(document.getElementsByName("add_money")[0].value)
      );
      break;
    case "add_service":
      ...
      $($(".add_money_span")[1]).html(
        comma(number_format(document.getElementsByName("add_money")[1].value))
      );
      break;
  }
  //console.log(($('#money1').val()*1)+($('#money2').val()*1));
  ...
  //$($(".add_money_span")[2]).	html(number_format(price));
  ...
  for (var i = 0; i < type_arr.length; i++) {
    if (type_arr[i].checked) {
      if (i == 0) {
        ...
        var goodname =
          "온리원문자 - 직접결제 - " + $("#add_term").val() + "개월기간 ";
        $($(".auto_pay_c")[0]).hide();
      } else {
        document.getElementsByName("mid")[0].value = "obmms20152";
        var goodname =
          "온리원문자 - 자동결제 - " + $("#add_term").val() + "개월기간 ";
        $($(".auto_pay_c")[0]).show();
      }
...
function pay_total_v(status) {
  ...
  if ($("input[name=money_type]:checked").val() * 1 == 3) {
    $("#total_amount").html(
      $("#add_phone").val() *
        1 *
        9900 *
        ($("input[name=money_type]:checked").val() * 1) *
        0.95
    );
    $("#price").val(
      $("#add_phone").val() *
  ...
  for (var i = 0; i < type_arr.length; i++) {
    if (type_arr[i].checked) {
      ...
      document.getElementsByName("goodname")[0].value = goodname;
      document.getElementsByName("max_cnt")[0].value =
        $("#add_phone").val() * 1 * 9000;
      document.getElementsByName("month_cnt")[0].value =
        $("input[name=money_type]:checked").val() * 1;
    }
  }
}
...
function pay_go(frm) {
  ...
  //if(document.getElementsByName('add_service')[0].checked)
  //	document.getElementsByName('fujia_status')[0].value='Y';
  //else
  //	document.getElementsByName('fujia_status')[0].value='';
  if (
    !document.getElementsByName("price")[0].value ||
    parseInt(document.getElementsByName("price")[0].value) <= 0
  ) {
      ...
      if (frm.mid.value == "obmms20151") {
        ...
        /*
    		...
    		$.ajax({
    			 ...
    			 url:"ajax/ajax_session.php",
    			 data:{
    				 	pay_go_mid:frm.mid.value,
    					pay_go_add_phone:frm.add_phone.value,
    					pay_go_month_cnt:frm.month_cnt.value,
    					pay_go_fujia_status:frm.fujia_status.value,
    					pay_go_goodname:frm.goodname.value,
    					pay_go_total_price:frm.price.value
    				  },
...
function board_save(frm, no, category) {
  ...
  if (frm.email_1)
    email_s = frm.email_1.value + "@" + frm.email_2.value + frm.email_3.value;
  if (frm.status_1) status_1_s = frm.status_1.checked ? "Y" : "N";
  if (document.getElementsByName("board_write_form_memo_hid")[0]) {
    adjunct_1_s = document.getElementsByName("board_write_form_img_hid")[0]
      .value;
    adjunct_2_s = document.getElementsByName("board_write_form_img_hid_2")[0]
      .value;
    var memo_name_arr =
      window.frames["upload_iframe"].document.getElementsByName("adjunct_memo");
    var adjunct_memo_a = [];
  ...
  fl_s = $("select[name=fl] option:selected").val();
  //alert(fl_s);
  if (confirm(msg)) {
    $($(".loading_div")[0]).show();
    $.ajax({
...
function board_save_reply(frm, no, category) {
  ...
  if (document.getElementsByName("fl")[0]) {
    ...
    for (var i = 0; i < fl_arr.length; i++) {
      if (fl_arr[i].checked) fl_s = fl_arr[i].value;
    }
    if (fl_s == "") {
      alert("분류 필수선택입니다.");
...
function send_delay(frm, tthis, status) {
  if (isNaN(tthis.value)) tthis.value = "";
  if (status == 2) {
    if (parseInt(tthis.value) <= parseInt(frm.delay.value)) tthis.value = "";
  }
}
//입력상태체크
function type_check() {
  ...
  else $($(".type_icon")[0]).children().hide();

  if (document.getElementsByName("upimage_str")[0].value)
    $($(".type_icon")[1]).children().show();
  else $($(".type_icon")[1]).children().hide();

  if (document.getElementsByName("onebook_status")[0].value == "Y")
    $($(".type_icon")[2]).children().show();
  else $($(".type_icon")[2]).children().hide();

  if (
    document.getElementsByName("ssh_check")[0].checked ||
    document.getElementsByName("ssh_check")[1].checked
  )
    $($(".type_icon")[3]).children().show();
  else $($(".type_icon")[3]).children().hide();

  if (
    document.getElementsByName("deny_wushi[]")[0].checked ||
    document.getElementsByName("deny_wushi[]")[1].checked ||
    document.getElementsByName("deny_wushi[]")[2].checked ||
    document.getElementsByName("deny_wushi[]")[3].checked
  )
    $($(".type_icon")[4]).children().show();
  else $($(".type_icon")[4]).children().hide();

  if (document.getElementsByName("rday")[0].value)
    $($(".type_icon")[5]).children().show();
  else $($(".type_icon")[5]).children().hide();
}
//결제해지
