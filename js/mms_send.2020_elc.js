...
{
    //console.log(chk_time+"^"+$('#close').val()+"^"+hour);
    // if($('#time_limit').is(":checked") == false) {
    //     if(chk_time == false) {
    //         if($('#close').val() != "") {
    //             if((hour *1) >= ($('#close').val() *1) || (hour *1) < ("08" *1)) {
    //         		alert('문자 발송가능 시간은 08시~'+$('#close').val()+'시 입니다.\n발송가능시간이 초과되었습니다. ');
    //         		return false;        
    //             }
    //         }
    //     }
    ...
    */   
	...
	if(error_phone == "") error_phone = "없음";
	    ...
	    add_msg = "수신처 초과 발송폰이 있습니다. 확인이 필요합니다.\n";
	}
    ...
    if($('#rday').val() != "") {
        send_msg += "[발송예정일 : "+$('#rday').val().substring(0,4)+"년 "+$('#rday').val().substring(5,7)+"월 "+$('#rday').val().substring(8,10)+"일 ]\n\n";     
    }
