...
<script>
    if ("<?= $platform ?>" != "mobile" && navigator.userAgent.toLocaleLowerCase().search("android") > -1) {
        location.href = "?platform=mobile&item_price=" + '<?= $item_price ?>' + '&item_name=' + '<?= $item_name ?>' + '&manager=' + '<?= $manager ?>' + '&conidx=' + '<?= $conidx ?>';
    }
</script>
<?
...
<script language=Javascript>
    $(function() {
        ...
        $("#item_count").on('change', function() {
            var settle_cash = $("#item_price_init").val();
            var sale_cnt = $("#sale_cnt").val();
            if (sale_cnt == '0') {
                var item_count = $("#item_count").val();
                if (item_count == "" || item_count == 0) {
                    $("#item_price").val(settle_cash);
                } else {
                    $("#item_price").val(settle_cash * $("#item_count").val());
                }
            } else {
                var item_count = $("#item_count").val();
        ...
        <div id="wrap" class="common-wrap" background-image="/images/main_bg_09.jpg" style="max-width: 1024px;padding:0px">
            ...
            <div class="pay" style="border: 1px solid #ddd;">
                <form name="pay_form" id="pay_form" method="post"> <!--승인요청 및 결과수신페이지 지정 //-->
                    ...
                    <input type="hidden" name="spgd" id="spgd" value="Y">
                    <div class="table-wrap">
                        <div id="spec_table" class="table-type">
                            <div class="row">
                                <div class="col-6" style="float: none">
                                    <input type="hidden" id="sale_cnt" value='<?= $con_sale_cnt ?>'>
                                    <input type="hidden" id="item_price_init" value='<?= $item_price ?>'>
                                    ...
                                    <div style="text-align: center;border: 2px solid;border-color: #cfcfcf">
                                        ...
                                        <div style="padding-top: 30px">
                                            <div style="display: flex;padding: 10px">
                                                ...
                                                <div><input type="number" name="item_count" id="item_count" style="border-radius: 5px;line-height: 20px;text-align: center;margin-left: 14px" value="1" min="1"></div>
                                            </div>
                                        </div>
