          ...
          <table class="list_table" width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            ?>
              ...
              while ($row = mysqli_fetch_array($result)) {
                ...
                $sql = "select count(*) as cnt, title, step from Gn_event_sms_step_info where sms_idx='{$row['sms_idx']}'";
...
<Script>
  function useIt(ori_sms_idx, event_name_eng, pcode, step, event_idx, title, mobile) {
    try {
      opener.$('#step_title').val(title);
    } catch (e) {

    }
    try {
      opener.$('#step_phone').val(mobile);
    } catch (e) {

    }
    try {
      opener.$('#step_cnt').val(step + "회");
    } catch (e) {
