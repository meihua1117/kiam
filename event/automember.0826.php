	...
	<body class="body-event">
									...
									<section class="input-field">												
										<section class="agreement-field">
											...
											<div class="container" style="margin-top: 20px;text-align: center;width: 100%;">
												<a class="auto_iam settlement_btn" href="http://obmms.net/iam?cur_win=best_sample">추천IAM보기</a>
											</div>
											<div class="container" style="margin-top: 20px;text-align: center;width: 100%;">
												<?
												if($button_txt != ""){
												?>
												<a class="auto_iam settlement_btn" href="<?=$event_data['event_sms_desc']?>" style="margin-left:25px;" target="_blank"><?=$button_txt?></a>
												<?}?>
											</div>
											<div class="container" style="margin-top: 20px;width: 100%;height:105px;">
												...
												<div class="mem_info">
													...
													<input type="text" style="font-size:18px;" id="mem_id" placeholder="소문자로 입력해주세요">
												</div>
		...
		<script>
			...
			function makingiam(){
				...
				if($("#checkAll_title").prop('checked') == false){
					...
					$.ajax({
						...
						data:{mem_name:mem_name, mem_phone:mem_phone, mem_id:mem_id, site:site, site_iam:site_iam, recom_id:recom_id, card_short_url:card_short_url, event_id:<?=$event_idx?>},
			...
			function gotoLogin() {
				...
				$('input[name=one_id]').val();
				$('input[name=mem_pass]').val();
				$('input[name=mem_code]').val();
