...
if ($_POST['mode'] == "speech") {
	if ($_POST['name'] == "") {
		...
		if ($mem_id != "") {
			...
			if ($event_data['reserv_type']) 
				$sql = "SELECT sms_idx FROM Gn_aievent_ms_info WHERE sms_idx='{$step_idx1}' or sms_idx='{$step_idx2}' or sms_idx='{$step_idx3}'";
			else
			...
			while ($lrow = mysqli_fetch_array($lresult)) {
				...
				//알람등록
				if ($event_data['reserv_type']) 
					$sql = "SELECT * FROM Gn_aievent_message WHERE sms_idx='{$sms_idx}'";
				else
				...
				if ($event_data['reserv_type'])
					$sql = "UPDATE Gn_aievent_request SET step_end_time='{$step_end_time}' WHERE request_idx = {$request_idx}";
				else
			...
			if ($stop_event_idx != 0) {
				...
				if ($cnt_row[0] > 0) {
					//중단정보테이블에 해당 값을 저장한다.
					if ($event_data['reserv_type'])
						$query = "INSERT INTO Gn_aievent_message SET event_idx='{$stop_event_idx}', mobile='{$recv_num}'";
					else
									...
									<section class="input-field">
										...
										<div class="form-wrap">
											...
											<? for ($i = 1; $i <= $event_data['file_cnt']; $i++) { ?>
												<div class="attr-row is-account ai" id="<?= 'ai_file_div' . $i ?>">
